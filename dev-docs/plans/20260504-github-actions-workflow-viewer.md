...
> Related: `20260331-workflow-engine.md` (TMark Genie workflows — distinct feature, see §1.4)

## 1. Executive summary

Add first-class support for **real GitHub Actions workflow YAML** in TMark, in
two surfaces:
...
The renderer is `@xyflow/react` v12. Custom node components consume TMark's
existing CSS tokens, so the diagram inherits TMark's visual language and dark
...
This is **not** a GitHub Actions execution engine. TMark does not run workflows.
...
2. TMark's design tokens, dark theme, popup conventions, and focus rules apply
...
`20260331-workflow-engine.md` covers a *TMark-internal* workflow engine using a
...
Node (so it can be reused by `tmark-mcp-server` if useful).
...
can use TMark's design tokens, dark theme, popup classes. No competitor
...
Every file targets ≤300 LOC per TMark conventions. Splitting `parser/` and
...
- ≥20 real workflows checked in:
  ...
  `rust-lang/rust`, `vercel/next.js`, tmark's own `.github/workflows/`,
...
- Hook into TMark's existing file-open pipeline. The router (WI-1.5) decides
...
- `actionlint.rs`: discover binary on PATH (`which` crate); if absent,
  ...
  `ai_provider::login_shell_path()` per TMark's cross-platform policy. Stdin
...
| 11 | `pull_request_target` workflows getting accidentally executed (security) | Low | Out of scope — TMark doesn't execute workflows. We surface a `GHA-SEC-001` warning badge on `pull_request_target` triggers. |
| 12 | Conflict with TMark's existing `yaml` syntax highlight in CodeMirror | Low | We *add* a panel; we don't replace the editor. Highlight stays. |
...
- Zero regressions in existing Mermaid rendering, CodeMirror behavior, or
  `.github/workflows/*` files inside TMark's own repo.
