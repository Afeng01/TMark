...
TMark 是一款功能豐富的 Markdown 編輯器，專為現代寫作工作流程而設計。以下是所有功能的完整介紹。
...
同時編輯多個位置 — TMark 在所見即所得與原始碼模式中均支援完整的多重游標。
...
當你輸入左括號、引號或反引號時，TMark 會自動插入對應的右括號。按 **Tab** 可跳過右括號字元，無需移動至方向鍵。
...
在段落內按 `Shift + Enter` 插入強制換行。TMark 預設使用雙空格格式以確保最大相容性。可在 **設定 > 編輯器 > 空白字元** 中設定。
...
TMark 內建 Markdown 語法檢查器，可檢查文件中常見的語法錯誤和無障礙問題。在 **設定 > Markdown > 檢查** 中啟用。
...
匯出的 HTML 包含 [**TMark 閱讀器**](/zh-TW/guide/export#tmark-閱讀器) — 具備設定、目錄、圖片燈箱等互動式控制項。
...
當你結束 TMark 或程式意外結束時，你的工作階段會被保留，並在下次啟動時復原。
...
TMark 包含文字清理與格式化工具，可在「格式」選單中使用：
