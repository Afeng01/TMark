...
TMark 支援 [Mermaid](https://mermaid.js.org/) 圖表，讓你直接在 Markdown 文件中建立流程圖、時序圖和其他視覺化圖表。
...
Mermaid 圖表會自動適應 TMark 目前的主題（White、Paper、Mint、Sepia 或 Night）。
...
TMark 使用 **Mermaid v11**，其解析器（Langium）比舊版更嚴格。AI 工具（ChatGPT、Claude、Copilot 等）常生成適用於舊版 Mermaid 的語法，但在 v11 中會失敗。以下是最常見的問題及修復方式。
...
TMark 渲染標準的 Mermaid 語法。要掌握圖表建立技巧，請參閱官方 Mermaid 文件：
...
- **[Mermaid 線上編輯器](https://mermaid.live/)** — 互動式測試場，在貼入 TMark 前測試和預覽圖表
- **[Mermaid 文件](https://mermaid.js.org/)** — 包含所有圖表類型範例的完整參考

::: tip
線上編輯器非常適合試驗複雜圖表。待圖表效果滿意後，再將程式碼複製到 TMark。
:::
