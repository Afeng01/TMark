...
TMark 在每個終端機工作階段中設定以下環境變數：

| 變數 | 值 |
|------|----|
| `TERM_PROGRAM` | `tmark` |
| `EDITOR` | `tmark` |
| `TMARK_WORKSPACE` | 工作區根目錄路徑（開啟資料夾時） |
...
對於產生大量輸出的長時間執行程序，你可以從 TMark 暫停底層的 shell 程序以釋放 CPU，但不結束工作階段。繼續後程序會從上次中斷處接續執行。
