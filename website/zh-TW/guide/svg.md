...
TMark 對 SVG（可縮放向量圖形）提供一流的支援。在文件中使用 SVG 有兩種方式，各適合不同的工作流程。
...
::: tip TMark 獨有功能
Typora 和 Obsidian 均不支援 `` ```svg `` 程式碼區塊。這是 TMark 的獨有功能，專為 AI 工作流程設計，讓工具生成的 SVG 視覺化內容（圖表、插圖、圖示）得以行內渲染。
:::
...
AI 程式設計助手可透過 MCP 工具直接將 SVG 生成至你的 TMark 文件。AI 發送帶有 `language: "svg"` 的程式碼區塊和 SVG 內容，自動行內渲染。
...
TMark 在渲染前對 SVG 內容進行清理。Script 標籤和事件處理器屬性（`onclick`、`onerror` 等）會被移除。這可防止貼上來自不受信任來源的 SVG 時發生 XSS 攻擊。
