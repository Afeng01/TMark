...
TMark 內建 MCP（模型情境協定）伺服器，讓 Claude 等 AI 助理能夠直接與你的編輯器互動。

## 什麼是 MCP？

[模型情境協定](https://modelcontextprotocol.io/)是一個開放標準，讓 AI 助理能夠與外部工具和應用程式互動。TMark 的 MCP 伺服器將其編輯器功能以工具的形式公開，供 AI 助理用來：
...
::: info 其他 MCP 相容用戶端
其他 MCP 相容用戶端（如 Cursor、Windsurf 等類似工具）也可以連接到 TMark 的 MCP 伺服器。透過指向 MCP 伺服器執行檔路徑來手動設定它們（請參閱下方的[手動設定](#手動設定)）。
:::
...
|------|------|------|
| ✓ 綠色 | 有效 | 設定正確且運作中 |
| ⚠ 琥珀色 | 路徑不符 | TMark 已移動 — 點擊 **修復** |
| ✗ 紅色 | 執行檔遺失 | 找不到 MCP 執行檔 — 重新安裝 TMark |
| ○ 灰色 | 未設定 | 尚未安裝 — 點擊 **安裝** |

::: tip TMark 移動了？
若你將 TMark.app 移至其他位置，狀態會顯示琥珀色「路徑不符」。只需點擊 **修復** 按鈕，以新路徑更新設定即可。
:::

### 3. 重新啟動你的 AI 助理

安裝或修復後，請 **完全重新啟動你的 AI 助理**（退出並重新開啟）以載入新設定。每次設定變更後，TMark 都會顯示提醒。
...
向 Claude 提問，讓它直接將答案寫入你的 TMark 文件：

<div class="screenshot-container">
  <img src="/screenshots/mcp-claude.png" alt="Claude Desktop using TMark MCP" />
  <p class="screenshot-caption">Claude Desktop 呼叫 <code>document</code> → <code>set_content</code> 寫入 TMark</p>
</div>

<div class="screenshot-container">
  <img src="/screenshots/mcp-result.png" alt="Content rendered in TMark" />
  <p class="screenshot-caption">內容立即出現在 TMark 中，格式完整</p>
</div>
...
::: tip 尋找執行檔路徑
在 macOS 上，MCP 伺服器執行檔位於 TMark.app 內：
...
在 Linux 上：
- `/usr/bin/tmark-mcp-server`（或你安裝的位置）
...
| 旗標 | 功能 |
|---|---|
| `--version`（或 `-v`） | 印出版本號（必須與執行中的 TMark 相符）並退出。 |
| `--health-check` | 對執行中的 TMark 橋接執行自我測試並退出。在接入 AI 助理之前，可用此指令驗證安裝是否正確。 |
...
tmark-mcp-server --port 9223   # 舊版 / 手動
```

## 運作原理

```text
AI 助理 <--stdio--> MCP 伺服器 <--WebSocket--> TMark 編輯器
```

1. **TMark 在啟動時** 在可用連接埠上啟動 WebSocket 橋接
2. **MCP 伺服器** 從 TMark 的應用程式資料目錄讀取連接埠和驗證權杖
3. **MCP 伺服器** 透過 WebSocket 橋接連線並驗證
4. **AI 助理** 透過 stdio 與 MCP 伺服器通訊
5. **指令透過橋接** 轉發至 TMark 的編輯器
...
若你將 TMark.app 移至其他位置（例如從下載移至應用程式），設定將指向舊路徑：
...
預設情況下，當 AI 助理修改你的文件（插入、取代或刪除內容）時，TMark 會建立需要你核准的 **建議**：
