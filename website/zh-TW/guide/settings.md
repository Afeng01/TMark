...
TMark 的設定面板讓你可以自訂編輯器的每個面向。使用 `Mod + ,` 或透過選單列中的 **TMark > 設定** 開啟。
...
| 貼上模式 | TMark 如何路由剪貼簿內容 | 智慧 | 智慧、純文字 |
...
| 設定 | 說明 | 預設 | 選項 |
|------|------|------|------|
| 檢查頻率 | 何時檢查新版 TMark | 啟動時 | 啟動時、每天、每週、手動 |
...
TMark 偵測 [Pandoc](https://pandoc.org) 以啟用匯出至其他格式（DOCX、EPUB、LaTeX 等）。點選 **偵測** 以掃描系統中的 Pandoc。若找到，會顯示其版本與路徑。
...
MCP（模型情境協定）伺服器允許 Claude Code 和 Cursor 等外部 AI 助理以程式方式控制 TMark。

| 設定 | 說明 | 預設 |
|------|------|------|
| 啟用 MCP 伺服器 | 啟動或停止 MCP 伺服器。執行中時，狀態徽章會顯示連接埠和已連線的用戶端 | 開啟（切換） |
| 啟動時自動開始 | TMark 開啟時自動啟動 MCP 伺服器 | 開啟 |
...
在 MCP 伺服器部分下方，你可以一鍵將 TMark 的 MCP 設定安裝至受支援的 AI 用戶端（Claude Desktop、Claude Code、Codex CLI、Gemini CLI）。
...
圖形介面設定會覆蓋所有環境變數 — 明確勝於隱含。留空可使用環境變數備用鏈 `$TMARK_EXTERNAL_EDITOR → $VISUAL → $EDITOR → 平台預設`。完整解析順序與安全閘道請見[在外部編輯器中開啟](/zh-TW/guide/formats#在外部編輯器中開啟)。

### 一次性升級提示

升級至多格式支援後首次啟動時，TMark 會顯示非阻斷性的提示通知，引導你前往**設定 → 格式**。此提示每次安裝只出現一次 — 顯示或關閉後不再重複出現。
...
| 設定 | 說明 | 預設 |
|------|------|------|
| 自訂連結協定 | 插入連結時 TMark 應識別的額外 URL 協定。以標籤形式輸入每個協定 | `obsidian`、`vscode`、`dict`、`x-dictionary` |

這讓你可以建立像 `obsidian://open?vault=...` 或 `vscode://file/...` 這樣的連結，TMark 會將其視為有效的 URL。
...
| 設定 | 說明 | 預設 | 平台 |
|------|------|------|------|
| 開啟時清除 macOS 隔離標記 | 開啟帶有 macOS 隔離屬性（`com.apple.quarantine`）的檔案時，先在讀取前剝除該屬性。對從網路下載而原本會被 TMark 阻擋的檔案有用 | 開啟 | macOS |
...
- [功能特色](/zh-TW/guide/features) — TMark 功能概覽
