...
TMark 中的工作區是以資料夾形式開啟的專案根目錄。開啟工作區後，側邊欄會顯示檔案樹狀結構、快速開啟功能可索引所有 Markdown 檔案、終端機從專案根目錄啟動，且你的開啟分頁會在下次啟動時還原。
...
| 拖放 | 從 Finder 將 Markdown 檔案拖入視窗 — TMark 自動偵測其專案根目錄並開啟工作區 |
| 最近的工作區 | **檔案 > 最近的工作區**，選取先前的專案 |

開啟工作區時，TMark 會在側邊欄顯示檔案總管。若之前已開啟過此工作區，先前開啟的分頁會自動還原。

::: tip
若目前視窗有未儲存的變更，TMark 會提示你在新視窗中開啟工作區，避免遺失目前的工作內容。
:::
...
開啟工作區時，TMark 可在 markdown 與文字檔的 **檔案內容**（而非僅檔名）中搜尋符合項目。
...
TMark 可記憶最多 10 個最近開啟的工作區。可從選單列的 **檔案 > 最近的工作區** 存取。
...
每個工作區都有自己的設定，在工作階段之間持久保留。設定儲存在 TMark 應用程式資料目錄中 — 不在專案資料夾內 — 所以你的工作區保持整潔。
...
關閉有工作區的視窗時，TMark 會將已開啟分頁的清單儲存至工作區設定。下次開啟同一工作區時，這些分頁會自動還原。
...
每個 TMark 視窗都可以有自己獨立的工作區。這讓你可以同時處理多個專案。

- **檔案 > 新視窗** 開啟全新視窗
- 在新視窗中開啟工作區不影響其他視窗
- 視窗大小和位置按視窗記憶

從 Finder 拖入 Markdown 檔案時，若目前視窗已有未儲存的工作，TMark 會自動在新視窗中開啟該檔案的專案。
...
TMark 會監控你的工作區，以偵測其他程式（Git、外部編輯器、建置工具等）所做的變更，並保持已開啟文件同步。
...
- 當多個已修改檔案同時變更時（例如 `git checkout` 後），TMark 會將它們批次合併至一個對話框，讓你可以全部重新載入、全部保留或逐一檢視。
- 若分歧檔案的磁碟內容後來與編輯器中的內容相符（例如 `git checkout` 還原了相同的文字），TMark 會自動清除分歧狀態，正常的自動儲存將恢復。

TMark 會過濾自身的儲存操作，因此你永遠不會因在應用程式內所做的變更而收到提示。

## macOS Dock 最近項目

你在 TMark 中開啟的文件會在 macOS 中註冊，因此當你在 Dock 中右鍵點擊 TMark 圖示時，它們會出現在 **最近使用的項目** 子選單中。

## 終端機整合

整合終端機會自動使用工作區根目錄作為其工作目錄。開啟或切換工作區時，所有終端機工作階段都會 `cd` 至新的根目錄。

在每個終端機工作階段中，`TMARK_WORKSPACE` 環境變數會被設定為工作區路徑，讓你的腳本可以引用專案根目錄。

[進一步了解終端機 →](/zh-TW/guide/terminal)

## Shell CLI 指令

TMark 可以安裝 `tmark` Shell 指令，讓你從終端機開啟檔案和資料夾。

### 安裝

前往 **說明 > 安裝 'tmark' 指令**。TMark 會在 `/usr/local/bin/tmark` 寫入一個小型啟動腳本，並要求你的管理員密碼（與 VS Code 安裝 `code` 指令的方式相同）。
...
該指令委派給 `open -b app.tmark`，因此 macOS 處理單一實例行為——檔案在你現有的 TMark 視窗中開啟，而不是啟動新程序。

### 解除安裝

前往 **說明 > 解除安裝 'tmark' 指令** 以移除 `/usr/local/bin/tmark`。若該路徑的檔案不是由 TMark 安裝的，操作將被封鎖，並提示你手動移除。
