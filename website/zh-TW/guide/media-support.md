...
TMark 支援在 Markdown 文件中嵌入影片、音訊和 YouTube，使用標準 HTML5 標籤。
...
你也可以直接貼上 Vimeo URL（例如 `https://vimeo.com/123456789`），TMark 會自動將其轉換為嵌入代碼。
...
貼上 Bilibili 影片 URL（例如 `https://bilibili.com/video/BV1xxxxxxxxx`），TMark 會自動將其轉換為嵌入代碼。請注意，短網址（`b23.tv`）不受支援，因為需要重新導向解析。

### 圖片語法後備

你也可以使用帶有媒體副檔名的圖片語法 — TMark 會自動將其升級為對應的媒體類型：
...
- **Vimeo** 和 **Bilibili** — 直接在編輯器中貼上影片 URL，TMark 會自動辨識平台

### 拖放

直接從檔案系統將影片或音訊檔案拖入編輯器。TMark 將：
