...
`{{content}}` 佔位符是每個精靈的核心。精靈執行時，TMark：
...
**自由格式提示** — 在選取器中輸入自由格式指令時，TMark 會自動為 `selection` 和 `block` 範圍包含 ±1 個相鄰區塊作為上下文。無需設定。
...
當精靈傳回的文字是用來取代選取範圍（而非自由格式聊天回覆）時，TMark 會將其呈現為帶行內差異的 **建議**：原始文字以紅色刪除線顯示，建議文字以綠色底線顯示。在任何變更實際保存之前，你需先審閱並核准。
