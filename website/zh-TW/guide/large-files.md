...
TMark 開啟大多數 Markdown 檔案都能瞬間完成，但極大型檔案需要額外處理才能維持流暢的反應。本頁說明 TMark 是如何處理這類檔案，以及你可以怎麼調整相關行為。

## 什麼算「大型」

TMark 會在開啟前先依大小將檔案分級：
...
| ≥ 50 MB | 拒絕 | TMark 拒絕開啟此檔案。請改用 `less`、`bat` 等工具檢視。 |
...
- **檔案在開啟期間變大。** TMark 是依開啟當下的大小決定級別。即使檔案在你編輯時膨脹到 2 MB，它仍會留在你選定的模式裡。
