...
TMark 包含一套全面的中文、日文和韓文排版規則。這些工具有助於在混用中日韓文與拉丁字元時保持一致的版面。
...
TMark 的 **拉丁文 Span 掃描器** 會自動偵測並保護技術結構，防止標點轉換：
...
TMark 包含專用的中日韓文字距功能，透過在字元之間加入細微間距來提升可讀性。
...
TMark 可在中日韓文格式化時自動將直引號轉換為排版正確的智慧引號，支援多種引號樣式。
...
將此未格式化的文字複製到 TMark，並按 `Alt + Mod + Shift + F` 進行格式化：
