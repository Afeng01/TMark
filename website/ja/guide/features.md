...
TMark は、モダンなライティングワークフロー向けに設計された機能豊富な Markdown エディタです。以下に含まれるものを説明します。
...
複数の場所を同時に編集 — TMark は WYSIWYG モードとソースモードの両方で完全なマルチカーソルをサポートします。
...
開き括弧、引用符、またはバッククォートを入力すると、TMark は自動的に閉じるペアを挿入します。**Tab** を押して矢印キーに手を伸ばす代わりに閉じる文字を飛び越えます。
...
TMark は最大互換性のためにデフォルトで 2 スペーススタイルを使用します。
...
TMark には、ドキュメントの一般的な構文ミスやアクセシビリティの問題をチェックする組み込み Markdown リンターが含まれています。**設定 > Markdown > Lint** で有効にできます。
...
TMark はドキュメントを共有するための柔軟なエクスポートオプションを提供します。
...
エクスポートされた HTML には[**TMark Reader**](/ja/guide/export#tmark-reader) — 設定、目次、画像ライトボックスなどのインタラクティブコントロールが含まれます。
...
TMark はドキュメントのスナップショットを自動的に保存するため、以前のバージョンを復元できます。
...
TMark を終了した場合や予期しない終了が発生した場合、セッションが保存され、次回起動時に復元されます。
...
**仕組み：**
- 終了時、TMark はすべてのウィンドウの完全なセッション状態をキャプチャ
...
TMark はフォーマットメニューで利用可能なテキストクリーンアップとフォーマットのユーティリティを含みます：
...
TMark は自動的にアップデートを確認し、アプリ内でダウンロードしてインストールできます：
