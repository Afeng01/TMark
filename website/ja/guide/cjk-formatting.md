...
TMark には中国語、日本語、韓国語テキストの包括的なフォーマットルールセットが含まれています。これらのツールは、CJK とラテン文字を混在させる際に一貫したタイポグラフィを維持するのに役立ちます。
...
TMark は **スタックベースの引用符ペアリングアルゴリズム** を使い、以下を正しく処理します：
...
TMark の **ラテンスパンスキャナー** は、句読点変換から技術的な構文を自動的に検出して保護します：
...
TMark には CJK テキスト専用の文字間隔機能が含まれており、文字間に微妙な間隔を追加することで可読性を向上させます。
...
TMark は CJK フォーマット中にストレート引用符を正しいスマート引用符に自動変換できます。この機能は複数の引用符スタイルをサポートします。
...
TMark は引用符のペアリングに洗練されたスタックベースのアルゴリズムを使用します：
...
このフォーマットされていないテキストを TMark にコピーして `Alt + Mod + Shift + F` を押してフォーマットしてみてください：
