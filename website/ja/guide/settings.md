...
TMark の設定パネルではエディタのすべての側面をカスタマイズできます。`Mod + ,` を押すか、メニューバーの **TMark > 設定** で開きます。
...
| 貼り付けモード | TMark がクリップボードのコンテンツをどうルーティングするか | Smart | Smart、Plain |
...
| 設定 | 説明 | デフォルト | オプション |
|-----|-----|---------|--------|
| 確認頻度 | TMark の新しいリリースをいつ確認するか | 起動時 | 起動時、毎日、毎週、手動 |
...
TMark は追加の形式（DOCX、EPUB、LaTeX など）へのエクスポートを有効にするために[Pandoc](https://pandoc.org)を検出します。**検出** をクリックしてシステム上の Pandoc をスキャンします。見つかった場合、バージョンとパスが表示されます。
...
MCP（Model Context Protocol）サーバーにより、Claude Code や Cursor などの外部 AI アシスタントが TMark をプログラムで制御できます。

| 設定 | 説明 | デフォルト |
|-----|-----|---------|
| MCP サーバーを有効化 | MCP サーバーを開始または停止します。実行中、ステータスバッジにポートと接続されているクライアントが表示されます | オン（切り替え） |
| 起動時に開始 | TMark 起動時に MCP サーバーを自動起動 | オン |
...
MCP サーバーセクションの下で、サポートされている AI クライアント（Claude Desktop、Claude Code、Codex CLI、Gemini CLI）に TMark の MCP 設定をワンクリックでインストールできます。
...
GUI 設定は環境変数より優先されます — 明示的なものが暗黙的なものに勝ちます。フィールドを空にすると、環境変数のフォールバックチェーン `$TMARK_EXTERNAL_EDITOR → $VISUAL → $EDITOR → プラットフォームデフォルト` を使用します。完全な解決順序とセキュリティゲートは[外部エディタで開く](/ja/guide/formats#外部エディタで開く)を参照してください。

### アップグレード時の通知

マルチフォーマットサポートへのアップグレード後の最初の起動時、TMark は**設定 → フォーマット**を指す非ブロッキングトーストを表示します。このお知らせはインストールごとに 1 回だけ表示されます — 一度表示（または閉じた）後は再表示されません。
...
| 設定 | 説明 | デフォルト |
|-----|-----|---------|
| カスタムリンクプロトコル | TMark がリンク挿入時に認識すべき追加の URL プロトコル。各プロトコルをタグとして入力 | `obsidian`、`vscode`、`dict`、`x-dictionary` |

これにより `obsidian://open?vault=...` または `vscode://file/...` のようなリンクを作成でき、TMark はそれらを有効な URL として扱います。
...
|-----|-----|---------|--------------|
| 開く時に macOS 検疫属性をクリア | macOS 検疫属性（`com.apple.quarantine`）を持つファイルを開くとき、読み取り前にそれを除去。Web からダウンロードして TMark が通常開けないファイルに役立ちます | オン | macOS |
...
- [機能](/ja/guide/features) — TMark の機能の概要
