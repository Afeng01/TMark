...
TMark はほとんどの Markdown ファイルを瞬時に開きますが、非常に大きなファイルは応答性を保つために追加の配慮が必要です。このページでは、TMark がそれらをどのように扱い、その動作をどのように調整できるかを説明します。

## 「大きい」とみなされる基準

TMark はファイルを開く前にサイズで分類します:
...
| ≥ 50 MB | 拒否 | TMark はファイルを開くのを拒否します。代わりに `less`、`bat` などのツールを使用してください。 |
...
- **開いている間にファイルが大きくなる。** TMark は開いた時点のサイズに基づいて区分を決定します。編集中に 2 MB に成長したファイルは、選択したモードのままになります。
