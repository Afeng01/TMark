...
TMark에는 중국어, 일본어, 한국어 텍스트를 위한 포괄적인 서식 규칙이 포함되어 있습니다. 이 도구들은 CJK와 라틴 문자를 혼용할 때 일관된 타이포그래피를 유지하는 데 도움이 됩니다.
...
TMark는 다음을 올바르게 처리하는 **스택 기반 따옴표 쌍 알고리즘** 을 사용합니다:
...
TMark의 **라틴 스팬 스캐너** 는 자동으로 기술 구문을 감지하고 구두점 변환으로부터 보호합니다:
...
TMark에는 CJK 텍스트 전용 자간 기능이 포함되어 있어 문자 사이에 미묘한 간격을 추가하여 가독성을 향상시킵니다.
...
TMark는 직선 따옴표를 타이포그래피적으로 올바른 스마트 따옴표로 자동 변환할 수 있습니다. 이 기능은 CJK 서식화 중에 작동하며 여러 따옴표 스타일을 지원합니다.
...
TMark는 따옴표 쌍을 위해 정교한 스택 기반 알고리즘을 사용합니다:
...
이 서식화되지 않은 텍스트를 TMark에 복사하고 `Alt + Mod + Shift + F`를 눌러 서식화합니다:
