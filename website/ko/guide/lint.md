...
TMark는 **스타일 선호도** 가 아니라 **정확성 문제** 를 잡아내는 내장 린트 엔진을 제공합니다. 린트는 요청 시 실행되며 (Cmd-Shift-L 또는 **도구 → Markdown 검사**), 결과는 인라인 거터 물결선으로 표시되고, 상태 표시줄 배지와 F2 탐색을 통해 발견 사항 사이를 이동할 수 있습니다.

## 린트가 하는 것과 하지 않는 것

TMark의 린트는 **정확성** 검사기입니다:
...
TMark의 린트는 스타일 강제 도구가 **아닙니다**. 다음은 보고하지 않습니다:
...
스타일 강제를 위해서는 TMark 외부에서 `prettier --check`와 같은 별도 도구를 사용하세요.
