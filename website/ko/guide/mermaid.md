...
TMark는 마크다운 문서에서 직접 순서도, 시퀀스 다이어그램 및 기타 시각화를 만들기 위해 [Mermaid](https://mermaid.js.org/) 다이어그램을 지원합니다.
...
TMark는 모든 Mermaid 다이어그램 타입을 지원합니다:
...
Mermaid 다이어그램은 TMark의 현재 테마 (White, Paper, Mint, Sepia, Night)에 자동으로 맞춰집니다.
...
TMark는 이전 버전보다 엄격한 파서 (Langium)를 가진 **Mermaid v11** 을 사용합니다. AI 도구 (ChatGPT, Claude, Copilot 등)는 종종 이전 Mermaid 버전에서는 작동했지만 v11에서는 실패하는 구문을 생성합니다. 다음은 가장 일반적인 문제와 해결 방법입니다.
...
TMark는 표준 Mermaid 구문을 렌더링합니다. 다이어그램 생성을 마스터하려면 공식 Mermaid 문서를 참조하세요:
...
- **[Mermaid Live Editor](https://mermaid.live/)** — TMark에 붙여넣기 전 다이어그램을 테스트하고 미리볼 수 있는 대화형 플레이그라운드
- **[Mermaid Documentation](https://mermaid.js.org/)** — 모든 다이어그램 타입에 대한 예시가 포함된 완전한 참조

::: tip
Live Editor는 복잡한 다이어그램을 실험하기에 좋습니다. 다이어그램이 올바르게 보이면 코드를 TMark에 복사하세요.
:::
