...
TMark는 현대적인 글쓰기 워크플로우를 위해 설계된 기능이 풍부한 마크다운 에디터입니다. 다음은 포함된 기능들입니다.
...
여러 위치를 동시에 편집합니다 — TMark는 WYSIWYG 및 소스 모드 모두에서 완전한 멀티 커서를 지원합니다.
...
여는 괄호, 따옴표 또는 백틱을 입력하면 TMark가 닫는 쌍을 자동 삽입합니다. **Tab** 을 눌러 화살표 키를 누르는 대신 닫는 문자를 건너뜁니다.
...
TMark는 최대 호환성을 위해 기본적으로 두 칸 스타일을 사용합니다.
**설정 > 에디터 > 공백** 에서 구성합니다.
...
TMark에는 문서의 일반적인 구문 오류와 접근성 문제를 검사하는 내장 마크다운 린터가 포함되어 있습니다. **설정 > 마크다운 > 린트** 에서 활성화합니다.
...
TMark는 문서를 공유하기 위한 유연한 내보내기 옵션을 제공합니다.
...
내보낸 HTML에는 설정, 목차, 이미지 라이트박스 등에 대한 대화형 컨트롤이 있는 [**TMark Reader**](/ko/guide/export#tmark-reader)가 포함됩니다.
...
TMark는 문서의 스냅샷을 자동으로 저장하여 이전 버전을 복구할 수 있습니다.
...
TMark를 종료하거나 예기치 않게 종료되면 세션이 보존되고 다음 실행 시 복원됩니다.
...
**작동 방식:**
- 종료 시 TMark가 모든 창의 전체 세션 상태를 캡처
...
TMark에는 서식 메뉴에서 사용할 수 있는 텍스트 정리 및 서식 유틸리티가 포함됩니다:
...
TMark는 자동으로 업데이트를 확인하고 앱 내에서 다운로드 및 설치할 수 있습니다:
