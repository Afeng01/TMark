...
TMark에는 Claude와 같은 AI 어시스턴트가 에디터와 직접 상호작용할 수 있게 해주는 내장 MCP (Model Context Protocol) 서버가 포함되어 있습니다.

## MCP란?

[Model Context Protocol](https://modelcontextprotocol.io/)은 AI 어시스턴트가 외부 도구 및 애플리케이션과 상호작용할 수 있게 해주는 개방형 표준입니다. TMark의 MCP 서버는 에디터 기능을 AI 어시스턴트가 다음과 같은 작업에 사용할 수 있는 도구로 노출합니다:
...
TMark는 원클릭 설치로 AI 어시스턴트 연결을 쉽게 만들어 줍니다.
...
Cursor, Windsurf 등 기타 MCP 호환 클라이언트도 TMark의 MCP 서버에 연결할 수 있습니다. MCP 서버 바이너리 경로를 지정하여 수동으로 구성하세요 (아래 [수동 설정](#수동-설정) 참조).
:::
...
|--------|------|------|
| ✓ 초록 | 유효 | 설정이 올바르고 작동 중 |
| ⚠ 황색 | 경로 불일치 | TMark가 이동됨 — **복구** 클릭 |
| ✗ 빨강 | 바이너리 없음 | MCP 바이너리를 찾을 수 없음 — TMark 재설치 |
| ○ 회색 | 미설정 | 설치되지 않음 — **설치** 클릭 |

::: tip TMark를 이동했나요?
TMark.app을 다른 위치로 이동하면 상태가 황색 "경로 불일치"로 표시됩니다. **복구** 버튼을 클릭하기만 하면 새 경로로 설정이 업데이트됩니다.
:::

### 3. AI 어시스턴트 재시작

설치 또는 복구 후, **AI 어시스턴트를 완전히 재시작** 하세요 (종료 후 다시 열기). 새 설정을 로드합니다. TMark는 각 설정 변경 후 알림을 표시합니다.

### 4. 사용해 보기

AI 어시스턴트에서 다음과 같은 명령을 시도해 보세요:
- *"TMark 문서에 무엇이 있나요?"*
- *"양자 컴퓨팅 요약을 TMark에 작성해 주세요"*
- *"문서에 목차를 추가해 주세요"*

## 실제 사용 예시

Claude에게 질문하고 답변을 TMark 문서에 직접 작성하도록 합니다:

<div class="screenshot-container">
  <img src="/screenshots/mcp-claude.png" alt="Claude Desktop using TMark MCP" />
  <p class="screenshot-caption">Claude Desktop이 <code>document</code> → <code>set_content</code>를 호출하여 TMark에 씁니다</p>
</div>

<div class="screenshot-container">
  <img src="/screenshots/mcp-result.png" alt="Content rendered in TMark" />
  <p class="screenshot-caption">콘텐츠가 TMark에 즉시 나타나며 완전히 서식이 적용됩니다</p>
</div>
...
::: tip 바이너리 경로 찾기
macOS에서 MCP 서버 바이너리는 TMark.app 내부에 있습니다:
...
Linux:
- `/usr/bin/tmark-mcp-server` (또는 설치 위치)
...
| 플래그 | 설명 |
|---|---|
| `--version` (또는 `-v`) | 버전을 출력하고 종료합니다 (실행 중인 TMark 와 일치해야 합니다). |
| `--health-check` | 실행 중인 TMark 브리지를 대상으로 자가 테스트를 실행하고 종료합니다. AI 어시스턴트를 연결하기 전에 설치 상태를 검증할 때 사용합니다. |
...
tmark-mcp-server --port 9223   # 레거시 / 수동
```

## 작동 방식

```text
AI 어시스턴트 <--stdio--> MCP 서버 <--WebSocket--> TMark 에디터
```

1. **TMark가 WebSocket 브리지를 시작** 합니다 — 실행 시 사용 가능한 포트에서
2. **MCP 서버** 가 TMark의 앱 데이터 디렉터리에서 포트와 인증 토큰을 읽습니다
3. **MCP 서버** 가 WebSocket 브리지를 통해 연결 및 인증합니다
4. **AI 어시스턴트** 는 stdio를 통해 MCP 서버와 통신합니다
5. **명령이 브리지를 통해** TMark 에디터로 전달됩니다
...
TMark는 MCP 서버 상태를 확인하는 여러 방법을 제공합니다:
...
- TMark가 실행 중이고 문서가 열려 있는지 확인합니다
- 설정 → 통합에서 MCP 서버가 활성화되어 있는지 확인합니다
- MCP 브리지가 "실행 중" 상태인지 확인합니다
- 연결이 끊어진 경우 TMark를 재시작합니다

### TMark 이동 후 경로 불일치

TMark.app을 다른 위치로 이동한 경우 (예: 다운로드에서 응용 프로그램으로), 설정이 이전 경로를 가리킵니다:
...
- TMark에서 문서 탭이 활성 상태인지 확인합니다
- 에디터 영역을 클릭하여 포커스를 줍니다
- 일부 명령은 먼저 텍스트를 선택해야 합니다

## 제안 시스템 및 자동 승인

기본적으로 AI 어시스턴트가 문서를 수정할 때 (삽입, 바꾸기, 삭제), TMark는 승인이 필요한 **제안** 을 생성합니다:
