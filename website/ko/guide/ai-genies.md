...
TMark는 네 가지 카테고리에 걸쳐 13개의 지니를 제공합니다:
...
`{{content}}` 플레이스홀더는 모든 지니의 핵심입니다. 지니가 실행될 때 TMark는:
...
지니가 (자유 형식 채팅 응답이 아니라) 선택 영역에 대한 교체로 의도된 텍스트를 반환하면, TMark는 인라인 diff와 함께 **제안** 으로 표시합니다: 원본 텍스트는 빨간색 취소선으로, 제안된 텍스트는 녹색 밑줄로 표시됩니다. 변경 사항이 영구적으로 적용되기 전에 검토하고 승인하세요.
