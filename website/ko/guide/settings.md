...
TMark의 설정 패널에서 에디터의 모든 측면을 사용자 정의할 수 있습니다. `Mod + ,`로 열거나 메뉴 바의 **TMark > 설정** 을 통해 접근합니다.
...
| 붙여넣기 모드 | TMark가 클립보드에서 콘텐츠를 라우팅하는 방식 | 스마트 | 스마트, 일반 |
...
| 설정 | 설명 | 기본값 | 옵션 |
|------|------|--------|------|
| 확인 빈도 | TMark의 새 릴리스를 확인할 시점 | 시작 시 | 시작 시, 매일, 매주, 수동 |
...
TMark는 추가 형식 (DOCX, EPUB, LaTeX 등)으로 내보내기를 활성화하기 위해 [Pandoc](https://pandoc.org)을 감지합니다. **감지** 를 클릭하여 시스템에서 Pandoc을 스캔합니다. 발견되면 버전과 경로가 표시됩니다.
...
MCP (Model Context Protocol) 서버는 Claude Code와 같은 외부 AI 어시스턴트가 TMark를 프로그래밍 방식으로 제어할 수 있게 합니다.

| 설정 | 설명 | 기본값 |
|------|------|--------|
| MCP 서버 활성화 | MCP 서버를 시작하거나 중지합니다. 실행 중일 때 포트와 연결된 클라이언트를 보여주는 상태 배지가 표시됩니다 | 켜기 (토글) |
| 시작 시 실행 | TMark가 열릴 때 MCP 서버 자동 시작 | 켜기 |
...
MCP 서버 섹션 아래에서 단 한 번의 클릭으로 지원되는 AI 클라이언트 (Claude Desktop, Claude Code, Codex CLI, Gemini CLI)에 TMark의 MCP 설정을 설치할 수 있습니다.
...
GUI 설정이 환경 변수보다 우선합니다 — 명시적인 것이 암묵적인 것보다 우선합니다. 비워 두면 환경 변수 폴백 체인 `$TMARK_EXTERNAL_EDITOR → $VISUAL → $EDITOR → 플랫폼 기본값`을 사용합니다. 전체 해석 순서와 보안 게이트는 [외부 에디터에서 열기](/ko/guide/formats#외부-에디터에서-열기)를 참조하세요.

### 일회성 업그레이드 안내

다중 형식 지원으로 업그레이드한 후 첫 실행 시, TMark는 **설정 → 형식**으로 안내하는 비차단 토스트 알림을 표시합니다. 이 알림은 설치당 한 번만 표시됩니다 — 한 번 표시되거나 닫히면 다시는 나타나지 않습니다.
...
| 설정 | 설명 | 기본값 |
|------|------|--------|
| 사용자 정의 링크 프로토콜 | 링크를 삽입할 때 TMark가 인식해야 하는 추가 URL 프로토콜. 각 프로토콜을 태그로 입력합니다 | `obsidian`, `vscode`, `dict`, `x-dictionary` |

이를 통해 TMark가 유효한 URL로 처리할 `obsidian://open?vault=...` 또는 `vscode://file/...`와 같은 링크를 만들 수 있습니다.
...
| 설정 | 설명 | 기본값 | 플랫폼 |
|------|------|--------|---------|
| 열 때 macOS 격리 속성 제거 | macOS 격리 속성 (`com.apple.quarantine`)이 있는 파일을 열 때, 읽기 전에 속성을 제거합니다. 웹에서 다운로드한 파일이 그렇지 않으면 TMark에서 열 수 없는 경우 유용합니다 | 켜기 | macOS |
...
- [기능](/ko/guide/features) — TMark 기능 개요
