...
TMark에는 에디터를 떠나지 않고 명령을 실행할 수 있는 내장 터미널 패널이 포함되어 있습니다.
...
TMark는 모든 터미널 세션에서 다음 환경 변수를 설정합니다:

| 변수 | 값 |
|------|-----|
| `TERM_PROGRAM` | `tmark` |
| `EDITOR` | `tmark` |
| `TMARK_WORKSPACE` | 워크스페이스 루트 경로 (폴더가 열려 있을 때) |
...
장시간 실행되며 자세한 출력을 생성하는 프로세스의 경우, 세션을 종료하지 않고 TMark에서 기본 셸 프로세스를 일시 중지하여 CPU를 절약할 수 있습니다. 재개하면 프로세스가 중단된 지점부터 계속됩니다.
