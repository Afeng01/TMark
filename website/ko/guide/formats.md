...
TMark는 아래의 모든 파일 형식을 직접 열 수 있습니다. 차별화된 특징은 **스키마 인식 미리보기**입니다. 알려진 아티팩트 파일의 경우, 일반적인 JSON 트리가 아닌 *적합한* 뷰로 렌더링됩니다.
...
다중 형식 지원으로 업그레이드한 후 첫 실행 시, TMark는 **설정 → 형식**으로 안내하는 일회성 토스트 알림을 표시합니다. 알림을 닫았거나 새로 설치한 경우, 언제든지 **설정 → 형식** 패널에서 확인할 수 있습니다.
...
경로 또는 내용이 알려진 스키마와 일치할 때, TMark는 일반 트리 대신 적합한 뷰로 대체합니다.
...
- 네트워크 호출 없음 — TMark는 crates.io를 해석하지 않습니다.
...
2. `$TMARK_EXTERNAL_EDITOR` (프로젝트 수준 환경 변수 재정의)
3. `$VISUAL`
4. `$EDITOR`
...
TMark는 로그인 셸 PATH 를 통해 라우팅하므로, macOS GUI 앱에서 실행할 때 VS Code / Cursor / JetBrains 래퍼가 올바르게 해석됩니다.
...
- 정규화된 확장자가 TMark의 등록된 형식 집합에 없는 경로
- 정규 대상이 위 검사 중 하나를 통과하지 못하는 심볼릭 링크

손상된 웹뷰는 이 버튼을 사용하여 임의의 시스템 파일 (암호, 키 등) 에서 외부 에디터를 실행할 수 없습니다 — TMark가 스스로 열 수 있는 경로에 한해서만 가능합니다.
