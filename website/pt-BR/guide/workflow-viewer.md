...
O TMark renderiza YAML de workflows do GitHub Actions como um grafo acíclico dirigido (DAG) interativo e permite editar jobs, steps e triggers através de formulários estruturados — sem nunca perder comentários, âncoras ou formatação no arquivo subjacente.
...
2. **Blocos de código em Markdown**: quando um bloco com cerca em três crases `yaml` ou `yml` contém um workflow reconhecível, o TMark o renderiza como um DAG estilo Mermaid em linha, da mesma forma que blocos `mermaid` são renderizados.

## Arquivos de workflow independentes

Abra qualquer `.github/workflows/*.yml` no TMark. O painel lateral à direita abre automaticamente e mostra:
...
Quando você clica em Salvar, o TMark:
...
O TMark detecta a forma do workflow (`jobs:` de nível superior com `runs-on` por job) e renderiza o diagrama em linha. O diagrama é somente leitura — edite o fonte para alterar o workflow.

## Diagnósticos

O TMark expõe diagnósticos de parse + lint ao lado do fonte:
...
Para steps `uses:` que referenciam GitHub Actions públicas, o TMark pode buscar o `action.yml` de cada action para popular as descrições de inputs no editor estruturado. Isso é opt-in e fica em cache no disco por 24 horas.
...
O TMark não executa workflows do GitHub Actions. É um visualizador e editor — a execução continua sendo trabalho do GitHub. A funcionalidade é puramente para ler, revisar e escrever YAML de workflow.
