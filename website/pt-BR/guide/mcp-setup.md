...
O TMark inclui um servidor MCP (Model Context Protocol) integrado que permite que assistentes de IA como o Claude interajam diretamente com seu editor.

## O que é MCP?

O [Model Context Protocol](https://modelcontextprotocol.io/) é um padrão aberto que permite que assistentes de IA interajam com ferramentas e aplicativos externos. O servidor MCP do TMark expõe suas capacidades de edição como ferramentas que assistentes de IA podem usar para:
...
O TMark facilita a conexão de assistentes de IA com instalação em um clique.
...
- **Iniciar ao abrir** - Iniciar automaticamente quando o TMark abrir
...
Outros clientes compatíveis com MCP, como Cursor, Windsurf e ferramentas semelhantes, também podem se conectar ao servidor MCP do TMark. Configure-os manualmente apontando para o caminho do binário do servidor MCP (veja [Configuração Manual](#configuracao-manual) abaixo).
:::
...
| ⚠ Âmbar | Caminho Incorreto | TMark foi movido — clique em **Reparar** |
| ✗ Vermelho | Binário Ausente | Binário MCP não encontrado — reinstale o TMark |
| ○ Cinza | Não Configurado | Não instalado — clique em **Instalar** |

::: tip TMark foi movido?
Se você mover o TMark.app para um local diferente, o status mostrará âmbar "Caminho Incorreto". Simplesmente clique no botão **Reparar** para atualizar a configuração com o novo caminho.
:::

### 3. Reiniciar o Assistente de IA

Após instalar ou reparar, **reinicie seu assistente de IA** completamente (feche e reabra) para carregar a nova configuração. O TMark exibirá um lembrete após cada alteração de configuração.

### 4. Experimente

No seu assistente de IA, tente comandos como:
- *"O que está no meu documento TMark?"*
- *"Escreva um resumo sobre computação quântica no TMark"*
- *"Adicione um índice ao meu documento"*

## Veja em Ação

Faça uma pergunta ao Claude e peça que ele escreva a resposta diretamente no seu documento TMark:

<div class="screenshot-container">
  <img src="/screenshots/mcp-claude.png" alt="Claude Desktop using TMark MCP" />
  <p class="screenshot-caption">O Claude Desktop chama <code>document</code> → <code>set_content</code> para escrever no TMark</p>
</div>

<div class="screenshot-container">
  <img src="/screenshots/mcp-result.png" alt="Content rendered in TMark" />
  <p class="screenshot-caption">O conteúdo aparece instantaneamente no TMark, totalmente formatado</p>
</div>
...
No macOS, o binário do servidor MCP está dentro do TMark.app:
...
No Linux:
- `/usr/bin/tmark-mcp-server` (ou onde você o instalou)
...
| Flag | O que faz |
|---|---|
| `--version` (ou `-v`) | Exibe a versão (deve corresponder ao TMark em execução) e encerra. |
| `--health-check` | Executa um autoteste contra a ponte TMark em execução e encerra. Use isso para verificar sua instalação antes de conectar um assistente de IA. |
...
tmark-mcp-server --port 9223   # legado / manual
```

## Como Funciona

```text
Assistente de IA <--stdio--> Servidor MCP <--WebSocket--> Editor TMark
```

1. **O TMark inicia uma ponte WebSocket** em uma porta disponível ao ser lançado
2. **O servidor MCP** lê a porta e o token de autenticação do diretório de dados do aplicativo TMark
3. **O servidor MCP** se conecta e autentica via ponte WebSocket
4. **O assistente de IA** se comunica com o servidor MCP via stdio
5. **Os comandos são retransmitidos** ao editor do TMark através da ponte
...
O TMark oferece várias formas de verificar o status do servidor MCP:
...
- Certifique-se de que o TMark está em execução e tem um documento aberto
...
- Reinicie o TMark se a conexão foi interrompida

### Caminho incorreto após mover o TMark

Se você moveu o TMark.app para um local diferente (por exemplo, de Downloads para Aplicativos), a configuração apontará para o caminho antigo:
...
- Certifique-se de que uma aba de documento está ativa no TMark
...
Por padrão, quando assistentes de IA modificam seu documento (inserir, substituir ou excluir conteúdo), o TMark cria **sugestões** que requerem sua aprovação:
