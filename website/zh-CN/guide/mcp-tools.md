...
TMark 向 AI 助手暴露 **四个复合 MCP 工具**：`session`、`workspace`、`document` 和 `workflow`。它们合计提供 **14 个操作** —— 涵盖读写主轴、文件与窗口生命周期，以及对 GitHub Actions YAML 进行 CST 安全的编辑。

之前的 12 工具 / 76 操作接口已被精简，因为文档内的格式化工具（粗体、标题、表格等）与 AI 智能体通过 Markdown 往返已经能轻松完成的工作重复。完整的设计取舍参见 [MCP 精简方案](https://github.com/Afeng01/TMark/blob/main/dev-docs/plans/20260504-mcp-pruning.md)。
...
::: tip Mermaid 图表
通过 MCP 使用 AI 生成 Mermaid 时，建议安装 [mermaid-validator MCP 服务器](/zh-CN/guide/mermaid#mermaid-验证器-mcp-服务器语法检查) —— 它使用与 TMark 同款的 Mermaid v11 解析器，在图表进入文档前先捕获语法错误。
:::
...
应用一组 `IRPatch` 对象。补丁会经过 TMark 的 CST 感知变更器分发，从而保留注释、锚点以及键的顺序。对 YAML 文件直接 `document.write` 会丢掉这些信息。
