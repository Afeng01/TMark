...
TMark 支持 [Mermaid](https://mermaid.js.org/) 图表，可在 Markdown 文档中直接创建流程图、时序图和其他可视化图形。
...
Mermaid 图表会自动适应 TMark 的当前主题（White、Paper、Mint、Sepia 或 Night）。
...
TMark 使用 **Mermaid v11**，其解析器（Langium）比旧版本更严格。AI 工具（ChatGPT、Claude、Copilot 等）通常生成在旧版 Mermaid 中有效但在 v11 中失败的语法。以下是最常见的问题及修复方法。
...
TMark 渲染标准 Mermaid 语法。要精通图表创建，请参阅 Mermaid 官方文档：
...
- **[Mermaid 在线编辑器](https://mermaid.live/)**——交互式演练场，在粘贴到 TMark 之前测试和预览图表
- **[Mermaid 文档](https://mermaid.js.org/)**——所有图表类型的完整参考及示例

::: tip
在线编辑器非常适合试验复杂图表。一旦图表看起来正确，就把代码复制到 TMark 中。
:::
