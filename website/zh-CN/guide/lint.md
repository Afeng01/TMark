...
TMark 内置一套 lint 引擎，专门捕获 **正确性问题**，而非样式偏好。Lint 按需运行（Cmd-Shift-L 或 **工具 → 检查 Markdown**），结果以行号槽中的波浪线、状态栏徽章和 F2 导航的形式呈现。

## Lint 是什么，不是什么

TMark 的 lint 是 **正确性** 检查器：
...
样式强制请在 TMark 之外使用 `prettier --check` 等独立工具。
