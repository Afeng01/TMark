...
TMark 把 GitHub Actions 工作流 YAML 渲染成一张可交互的有向无环图（DAG），并允许你通过结构化表单编辑 jobs、steps 和 triggers —— 整个过程不会丢失底层文件中的注释、锚点或格式。

该功能在两个场景下都可用：

1. **独立的 `.yml` 文件**，位于 `.github/workflows/` 下（或任何顶层结构能被识别为 workflow 的文件）：分屏视图，左侧是源码，右侧是可交互画布加上结构化编辑器。
2. **Markdown 中的代码围栏**：当一个三反引号 `yaml` 或 `yml` 围栏块中含有可识别的 workflow 时，TMark 会像渲染 `mermaid` 块那样，将其渲染为内联的 Mermaid 风格 DAG。

## 独立 workflow 文件

在 TMark 中打开任意 `.github/workflows/*.yml` 文件。右侧面板会自动展开，显示：
...
`with:` 块以"键/值"行的增/改/删形式呈现。重命名一个键时，TMark 会先对旧键发出 `with.remove`，再对新键发出 `with.set`。
...
TMark 会识别 workflow 的形态（顶层有 `jobs:`，每个 job 都有 `runs-on`），并以内联方式渲染图表。该图表是只读的 —— 想改 workflow，请编辑源码。

## 诊断信息

TMark 会在源码旁边展示解析与 lint 诊断：
...
对于引用了公共 GitHub Action 的 `uses:` step，TMark 可以拉取每个 action 的 `action.yml`，把输入项的描述填入结构化编辑器。该选项默认关闭，启用后会在本地缓存 24 小时。
...
TMark 不会执行 GitHub Actions 工作流。它只是一个查看器和编辑器 —— 执行依旧是 GitHub 的事。整套功能只面向阅读、审阅和编写 workflow YAML。
