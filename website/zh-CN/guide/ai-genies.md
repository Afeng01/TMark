...
`{{content}}` 占位符是每个精灵的核心。精灵运行时，TMark 会：
...
**自由格式提示词**——当你在选择器中输入自由格式指令时，TMark 自动包含 ±1 个周边块作为 `selection` 和 `block` 范围的上下文，无需配置。
...
当 Genie 返回的文本意图作为选区的替换（而非自由格式的对话回复）时，TMark 会以 **建议** 的形式展示它，附带内联差异：原文红色删除线，提议文本绿色下划线。在任何更改持久化之前，你都需要审核并批准。
