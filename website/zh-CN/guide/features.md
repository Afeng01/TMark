...
TMark 是一款功能丰富的 Markdown 编辑器，专为现代写作工作流而设计。以下是全部功能介绍。
...
同时编辑多个位置 —— TMark 在所见即所得和源码模式下均支持完整的多光标功能。
...
当你输入开括号、引号或反引号时，TMark 会自动插入对应的闭合字符。按 **Tab** 可跳过闭合字符，无需伸手去按方向键。
...
TMark 内置 Markdown 语法检查器，可检查文档中常见的语法错误和无障碍问题。在 **设置 > Markdown > 检查** 中启用。
...
导出的 HTML 包含 [**TMark 阅读器**](/zh-CN/guide/export#tmark-reader) —— 提供设置、目录、图片灯箱等交互控件。
...
当你退出 TMark 或程序意外退出时，你的会话会被保留，并在下次启动时恢复。
