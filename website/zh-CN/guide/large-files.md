...
TMark 能秒开绝大多数 Markdown 文件，但是非常大的文件需要额外处理才能保持响应流畅。本页介绍 TMark 是如何处理这类文件的，以及你如何调整这一行为。

## 什么算"大文件"

TMark 在打开文件之前会先按文件大小分级：
...
| ≥ 50 MB | 拒绝 | TMark 拒绝打开。请改用 `less`、`bat` 或类似工具。 |
...
- **将超过 1 MB 的文件自动以源码模式打开** *（默认开启）* —— 关闭后，TMark 会让 5 MB 以下的文件都以 WYSIWYG 打开，代价是更长的打开时间。
...
- **文件在打开后变大。** TMark 是按打开时刻的大小决定分级的。一份你在编辑过程中长到 2 MB 的文件，会保持你最初选择的模式。
