...
TMark 内置了一套全面的中文、日文和韩文排版规则。这些工具有助于在混合中日韩与拉丁字符时保持一致的排版风格。
...
TMark 的 **拉丁文段落扫描器** 会自动检测并保护技术性结构，防止标点转换：
...
TMark 内置了专用的中日韩文字间距功能，通过在字符间添加细微间距来提高可读性。
...
TMark 可以在中日韩格式化时自动将直引号转换为排版正确的智能引号，并支持多种引号样式。
...
将以下未格式化的文本复制到 TMark 中，然后按 `Alt + Mod + Shift + F` 进行格式化：
