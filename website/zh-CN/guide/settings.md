...
TMark 的设置面板让你可以自定义编辑器的方方面面。使用 `Mod + ,` 或通过菜单栏中的 **TMark > 设置** 打开。
...
| 粘贴模式 | TMark 如何路由剪贴板中的内容 | 智能 | 智能、纯文本 |
...
| 设置 | 描述 | 默认值 | 选项 |
|------|------|--------|------|
| 检查频率 | 何时检查 TMark 新发布版本 | 启动时 | 启动时、每天、每周、手动 |
...
TMark 检测 [Pandoc](https://pandoc.org) 以支持导出为其他格式（DOCX、EPUB、LaTeX 等）。点击 **检测** 扫描你系统上的 Pandoc。找到后会显示其版本和路径。
...
MCP（模型上下文协议）服务器允许外部 AI 助手（如 Claude Code 和 Cursor）以编程方式控制 TMark。

| 设置 | 描述 | 默认值 |
|------|------|--------|
| 启用 MCP 服务器 | 启动或停止 MCP 服务器。运行时，状态徽章显示端口和已连接的客户端 | 开启（切换） |
| 启动时自动启动 | TMark 打开时自动启动 MCP 服务器 | 开启 |
...
在 MCP 服务器部分下方，你可以一键将 TMark 的 MCP 配置安装到支持的 AI 客户端（Claude Desktop、Claude Code、Codex CLI、Gemini CLI）。
...
图形界面设置优先于任何环境变量 —— 显式优于隐式。留空则使用环境变量回退链 `$TMARK_EXTERNAL_EDITOR → $VISUAL → $EDITOR → 平台默认`。完整的解析顺序和安全关口说明，请参阅[在外部编辑器中打开](/zh-CN/guide/formats#在外部编辑器中打开)。

### 一次性升级提示

首次升级至多格式支持后启动时，TMark 会显示一个无阻塞的提示，引导你前往 **设置 → 格式**。此提示每次安装只显示一次 —— 显示或关闭后不再出现。
...
| 设置 | 描述 | 默认值 |
|------|------|--------|
| 自定义链接协议 | 插入链接时 TMark 应识别的额外 URL 协议，以标签形式输入每个协议 | `obsidian`、`vscode`、`dict`、`x-dictionary` |

这让你可以创建如 `obsidian://open?vault=...` 或 `vscode://file/...` 这样的链接，TMark 会将其视为有效 URL。
...
| 设置 | 描述 | 默认值 | 平台 |
|------|------|--------|------|
| 打开时清除 macOS 隔离属性 | 当打开带有 macOS 隔离属性（`com.apple.quarantine`）的文件时，在读取前剥除该属性。对从网络下载、本来会被 TMark 阻止打开的文件很有帮助 | 开启 | macOS |
...
- [功能特性](/zh-CN/guide/features) —— TMark 功能概览
