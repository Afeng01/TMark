...
TMark 可直接打开以下所有文件格式。其差异化之处在于**架构感知预览**：当文件是已知的制品时，TMark 会渲染*正确*的视图，而不是通用的 JSON 树。
...
首次升级至多格式支持后启动时，TMark 会显示一次性提示，引导你前往 **设置 → 格式**。如果你已关闭提示（或全新安装），随时可在 **设置 → 格式** 中找到此面板。
...
当路径或内容匹配已知架构时，TMark 会以合适的视图替换通用树形显示。
...
2. `$TMARK_EXTERNAL_EDITOR`（项目级环境变量覆盖）
3. `$VISUAL`
4. `$EDITOR`
...
TMark 通过登录 Shell 的 PATH 进行路由，因此从 macOS 图形界面应用启动时，VS Code / Cursor / JetBrains 的包装程序也能正确解析。
...
- 不存在的路径
- 目录和其他非普通文件（套接字、设备）
- 规范化扩展名不在 TMark 已注册格式集中的路径
- 其规范化目标未通过上述任何检查的符号链接

即使 webview 遭到入侵，也无法通过此按钮在任意系统文件（密码、密钥等）上启动外部编辑器 —— 只能对 TMark 本身可打开的路径操作。
