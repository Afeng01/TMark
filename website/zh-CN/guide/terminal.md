...
TMark 在每个终端会话中设置以下环境变量：

| 变量 | 值 |
|------|-----|
| `TERM_PROGRAM` | `tmark` |
| `EDITOR` | `tmark` |
| `TMARK_WORKSPACE` | 工作区根路径（打开文件夹时） |
...
对于会产生大量输出的长时间运行进程，你可以从 TMark 中挂起底层的 shell 进程，以释放 CPU 而不杀死会话。恢复后进程会从中断处继续。
