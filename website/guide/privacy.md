...
TMark respects your privacy. Here's exactly what happens — and what doesn't.

## What TMark Sends

TMark includes an **auto-update checker** that periodically contacts our server to see if a new version is available. This is the **only** network request TMark makes.
...
You can verify this yourself — the endpoint is in [`tauri.conf.json`](https://github.com/Afeng01/TMark/blob/main/src-tauri/tauri.conf.json) (search for `"endpoints"`), and the hash is in [`lib.rs`](https://github.com/Afeng01/TMark/blob/main/src-tauri/src/lib.rs) (search for `machine_id_hash`).

## What TMark Does NOT Send
...
These numbers are published openly at [`configured stats endpoint`](https://github.com/Afeng01/TMark/releases). Nothing is hidden.
...
- There is no account system — TMark doesn't know who you are
...
TMark is fully open source. You can verify everything described here:

- Update endpoint configuration: [`src-tauri/tauri.conf.json`](https://github.com/Afeng01/TMark/blob/main/src-tauri/tauri.conf.json)
- Machine hash generation: [`src-tauri/src/lib.rs`](https://github.com/Afeng01/TMark/blob/main/src-tauri/src/lib.rs) — search for `machine_id_hash`
- Server-side stats aggregation: [`scripts/tmark-stats-json`](https://github.com/Afeng01/TMark/blob/main/scripts/tmark-stats-json) — the exact script that runs on our server to produce the [public stats](https://github.com/Afeng01/TMark/releases)
- No other network calls exist in the codebase — search for `fetch`, `http`, or `reqwest` yourself

## Disabling Update Checks

If you prefer to disable automatic update checks entirely, you can block the configured update endpoint at the network level (firewall, `/etc/hosts`, or DNS). TMark will continue to work normally without it — you just won't receive update notifications.
