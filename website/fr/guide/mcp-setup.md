...
TMark inclut un serveur MCP (Model Context Protocol) intégré qui permet aux assistants IA comme Claude d'interagir directement avec votre éditeur.

## Qu'est-ce que MCP ?

Le [Model Context Protocol](https://modelcontextprotocol.io/) est un standard ouvert qui permet aux assistants IA d'interagir avec des outils et applications externes. Le serveur MCP de TMark expose ses capacités d'éditeur comme outils que les assistants IA peuvent utiliser pour :
...
TMark facilite la connexion des assistants IA avec une installation en un clic.
...
<div class="screenshot-container">
  <img src="/screenshots/mcp-settings-server.png" alt="Paramètres du serveur MCP TMark" />
</div>

- **Activer le serveur MCP** - Activer pour autoriser les connexions IA
- **Démarrer au lancement** - Démarrage automatique à l'ouverture de TMark
...
<div class="screenshot-container">
  <img src="/screenshots/mcp-settings-install.png" alt="Installation de la configuration MCP TMark" />
</div>
...
D'autres clients compatibles MCP tels que Cursor, Windsurf et des outils similaires peuvent également se connecter au serveur MCP de TMark. Configurez-les manuellement en pointant vers le chemin du binaire du serveur MCP (voir [Configuration manuelle](#configuration-manuelle) ci-dessous).
:::
...
| ⚠ Ambre | Incompatibilité de chemin | TMark a été déplacé — cliquez sur **Réparer** |
| ✗ Rouge | Binaire manquant | Binaire MCP introuvable — réinstallez TMark |
| ○ Gris | Non configuré | Non installé — cliquez sur **Installer** |

::: tip TMark déplacé ?
Si vous déplacez TMark.app vers un autre emplacement, le statut affichera en ambre « Incompatibilité de chemin ». Cliquez simplement sur le bouton **Réparer** pour mettre à jour la configuration avec le nouveau chemin.
:::

### 3. Redémarrer votre assistant IA

Après l'installation ou la réparation, **redémarrez complètement votre assistant IA** (quittez et rouvrez) pour charger la nouvelle configuration. TMark affichera un rappel après chaque changement de configuration.

### 4. Essayer

Dans votre assistant IA, essayez des commandes comme :
- *« Qu'est-ce qu'il y a dans mon document TMark ? »*
- *« Écris un résumé sur l'informatique quantique dans TMark »*
- *« Ajoute une table des matières à mon document »*

## Le voir en action

Posez une question à Claude et faites-lui écrire la réponse directement dans votre document TMark :

<div class="screenshot-container">
  <img src="/screenshots/mcp-claude.png" alt="Claude Desktop utilisant TMark MCP" />
  <p class="screenshot-caption">Claude Desktop appelle <code>document</code> → <code>set_content</code> pour écrire dans TMark</p>
</div>

<div class="screenshot-container">
  <img src="/screenshots/mcp-result.png" alt="Contenu rendu dans TMark" />
  <p class="screenshot-caption">Le contenu apparaît instantanément dans TMark, entièrement formaté</p>
</div>
...
Sur macOS, le binaire du serveur MCP est à l'intérieur de TMark.app :
...
Sur Linux :
- `/usr/bin/tmark-mcp-server` (ou là où vous l'avez installé)
...
|---|---|
| `--version` (ou `-v`) | Affiche la version (doit correspondre à la TMark en cours d'exécution) et quitte. |
| `--health-check` | Exécute un auto-test contre le pont TMark en cours d'exécution et quitte. Utilisez ceci pour vérifier votre installation avant de connecter un assistant IA. |
...
tmark-mcp-server --port 9223   # legacy / manuel
```

## Fonctionnement

```text
Assistant IA <--stdio--> Serveur MCP <--WebSocket--> Éditeur TMark
```

1. **TMark démarre un pont WebSocket** sur un port disponible au lancement
2. **Le serveur MCP** lit le port et le jeton d'authentification depuis le répertoire de données de l'application TMark
...
5. **Les commandes sont relayées** vers l'éditeur de TMark via le pont
...
TMark fournit plusieurs façons de vérifier le statut du serveur MCP :
...
- Assurez-vous que TMark est en cours d'exécution et qu'un document est ouvert
...
- Redémarrez TMark si la connexion a été interrompue

### Incompatibilité de chemin après le déplacement de TMark

Si vous avez déplacé TMark.app vers un autre emplacement (ex. de Téléchargements vers Applications), la configuration pointera vers l'ancien chemin :
...
- Assurez-vous qu'un onglet de document est actif dans TMark
...
Par défaut, lorsque les assistants IA modifient votre document (insérer, remplacer ou supprimer du contenu), TMark crée des **suggestions** qui nécessitent votre approbation :
