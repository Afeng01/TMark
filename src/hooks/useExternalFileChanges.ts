...
import { isWatchIdForWindow } from "@/utils/workspaceWatchIds";
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      ...
      const unlisten = await listen<FsChangeEvent>("fs:changed", async (event) => {
        ...
        // Only process events from this window's watcher(s), including extra
        // workspace roots that use derived watch ids.
        if (!isWatchIdForWindow(watchId, windowLabel)) return;
