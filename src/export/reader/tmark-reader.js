/**
 * TMark Reader - Interactive controls for exported HTML
 *
 * Features:
...
(function() {
  ...
  // Font stacks (matching TMark editor fonts)
  const FONT_STACKS = {
    latin: {
  ...
  // Theme definitions (matching TMark editor themes)
  const THEMES = {
    white: {
      background: '#FFFFFF',
      foreground: '#1a1a1a',
  ...
  // Storage key
  const STORAGE_KEY = 'tmark-reader-settings';
  ...
  function loadSettings() {
    try {
      ...
      console.warn('[TMark Reader] Failed to load settings:', e);
    }
  }
  ...
  function saveSettings() {
    try {
      ...
      console.warn('[TMark Reader] Failed to save settings:', e);
    }
  }
  ...
  function applyCodeBlockButtons() {
    ...
    codeBlocks.forEach(wrapper => {
      ...
      btnContainer.className = 'tmark-code-btn-group';
      ...
      if (lineNumbers) {
        ...
        lineNumBtn.className = 'tmark-code-btn';
      ...
      copyBtn.className = 'tmark-code-btn';
      ...
      copyBtn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(code).then(() => {
          ...
          console.warn('[TMark Reader] Copy failed:', err);
        });
      });
  ...
  function createTocToggleTab() {
    ...
    tocToggleTab.className = 'tmark-toc-toggle-tab';
  ...
  function applyToc() {
    ...
    if (!settings.showToc) {
      ...
      if (tocSidebar) {
        ...
        document.body.classList.remove('tmark-toc-open');
      }
      if (tocBackdrop) {
    ...
    if (tocSidebar) {
      ...
      document.body.classList.add('tmark-toc-open');
    ...
    tocSidebar.className = 'tmark-toc-sidebar visible';

    // Header with close button (for mobile only)
    const header = document.createElement('div');
    header.className = 'tmark-toc-header';
    header.innerHTML = `<button class="tmark-toc-close" title="Close">&times;</button>`;
    tocSidebar.appendChild(header);

    // Navigation
    const nav = document.createElement('nav');
    nav.className = 'tmark-toc-nav';

    tocHeadings.forEach((item, index) => {
      const link = document.createElement('a');
      link.href = `#${item.id}`;
      link.className = `tmark-toc-item tmark-toc-level-${item.level}`;
    ...
    tocSidebar.querySelector('.tmark-toc-close').addEventListener('click', closeToc);

    // Create backdrop for mobile
    tocBackdrop = document.createElement('div');
    tocBackdrop.className = 'tmark-toc-backdrop';
  ...
  function handleScrollSpy() {
    ...
    // Update active state
    const links = tocSidebar.querySelectorAll('.tmark-toc-item');
    links.forEach((link, index) => {
      link.classList.toggle('active', index === currentIndex);
    });

    // Scroll TOC to keep active item visible
    const activeLink = tocSidebar.querySelector('.tmark-toc-item.active');
    if (activeLink) {
      const nav = tocSidebar.querySelector('.tmark-toc-nav');
  ...
  function createPanel() {
    ...
    toggle.className = 'tmark-reader-toggle';
    ...
    panel.className = 'tmark-reader-panel';
    panel.innerHTML = `
      <div class="tmark-reader-header">
        <span>Reader Settings</span>
        <button class="tmark-reader-close" title="Close">&times;</button>
      </div>
      <div class="tmark-reader-content">
        <div class="tmark-reader-group">
          <label>Font Size</label>
          <div class="tmark-reader-range-row">
            <button class="tmark-reader-btn" data-action="fontSize" data-dir="-1">−</button>
            <span class="tmark-reader-value" data-value="fontSize">${settings.fontSize}px</span>
            <button class="tmark-reader-btn" data-action="fontSize" data-dir="1">+</button>
          </div>
        </div>
        <div class="tmark-reader-group">
          ...
          <div class="tmark-reader-range-row">
            <button class="tmark-reader-btn" data-action="lineHeight" data-dir="-1">−</button>
            <span class="tmark-reader-value" data-value="lineHeight">${settings.lineHeight}</span>
            <button class="tmark-reader-btn" data-action="lineHeight" data-dir="1">+</button>
          </div>
        </div>
        <div class="tmark-reader-group">
          ...
          <div class="tmark-reader-range-row">
            <button class="tmark-reader-btn" data-action="contentWidth" data-dir="-1">−</button>
            <span class="tmark-reader-value" data-value="contentWidth">${settings.contentWidth}em</span>
            <button class="tmark-reader-btn" data-action="contentWidth" data-dir="1">+</button>
          </div>
        </div>
        <div class="tmark-reader-group">
          <label>Latin Font</label>
          <select class="tmark-reader-select" data-setting="latinFont">
        ...
        <div class="tmark-reader-group">
          ...
          <select class="tmark-reader-select" data-setting="cjkFont">
            ${FONT_OPTIONS.cjk.map(o => `<option value="${o.value}" ${settings.cjkFont === o.value ? 'selected' : ''}>${o.label}</option>`).join('')}
          </select>
        </div>
        <div class="tmark-reader-group">
          <label>Theme</label>
          <div class="tmark-reader-theme-row">
            <button class="tmark-reader-theme-circle ${settings.theme === 'white' ? 'active' : ''}" data-theme="white" title="White" style="background: ${THEMES.white.background}"></button>
            <button class="tmark-reader-theme-circle ${settings.theme === 'paper' ? 'active' : ''}" data-theme="paper" title="Paper" style="background: ${THEMES.paper.background}"></button>
            <button class="tmark-reader-theme-circle ${settings.theme === 'mint' ? 'active' : ''}" data-theme="mint" title="Mint" style="background: ${THEMES.mint.background}"></button>
            <button class="tmark-reader-theme-circle ${settings.theme === 'sepia' ? 'active' : ''}" data-theme="sepia" title="Sepia" style="background: ${THEMES.sepia.background}"></button>
            <button class="tmark-reader-theme-circle theme-night ${settings.theme === 'night' ? 'active' : ''}" data-theme="night" title="Night" style="background: ${THEMES.night.background}"></button>
          </div>
        </div>
        <div class="tmark-reader-group">
          <label>CJK Letter Spacing</label>
          <div class="tmark-reader-range-row">
            <button class="tmark-reader-btn" data-action="cjkLetterSpacing" data-dir="-1">−</button>
            <span class="tmark-reader-value" data-value="cjkLetterSpacing">${settings.cjkLetterSpacing}em</span>
            <button class="tmark-reader-btn" data-action="cjkLetterSpacing" data-dir="1">+</button>
          </div>
        </div>
        <div class="tmark-reader-group">
          <label class="tmark-reader-checkbox-label">
        ...
        <div class="tmark-reader-group tmark-reader-reset">
          <button class="tmark-reader-reset-btn" data-action="reset">Reset to Defaults</button>
        </div>
      </div>
    `;

    // Event listeners
    panel.querySelector('.tmark-reader-close').addEventListener('click', togglePanel);

    panel.querySelectorAll('.tmark-reader-btn').forEach(btn => {
      btn.addEventListener('click', handleRangeClick);
    });

    panel.querySelectorAll('.tmark-reader-theme-circle').forEach(btn => {
    ...
    panel.querySelectorAll('.tmark-reader-select').forEach(select => {
      select.addEventListener('change', handleSelectChange);
    });

    panel.querySelector('.tmark-reader-reset-btn').addEventListener('click', handleReset);
  ...
  function togglePanel() {
    ...
    document.body.classList.toggle('tmark-panel-open', isOpen);
  }
  ...
  function createBackToTop() {
    ...
    backToTopBtn.className = 'tmark-back-to-top';
  ...
  function createProgressBar() {
    ...
    progressBar.className = 'tmark-progress-bar';
    progressBar.setAttribute('role', 'progressbar');
    progressBar.setAttribute('aria-label', 'Reading progress');

    const progressFill = document.createElement('div');
    progressFill.className = 'tmark-progress-fill';
  ...
  function updateProgress() {
    ...
    const fill = progressBar.querySelector('.tmark-progress-fill');
    const scrollTop = window.scrollY;
  ...
  function setupImageLightbox() {
    ...
    lightbox.className = 'tmark-lightbox';
    lightbox.setAttribute('role', 'dialog');
    lightbox.setAttribute('aria-label', 'Image preview');
    lightbox.innerHTML = `
      <button class="tmark-lightbox-close" aria-label="Close">&times;</button>
      <img class="tmark-lightbox-img" src="" alt="">
    `;

    lightbox.addEventListener('click', (e) => {
      if (e.target === lightbox || e.target.classList.contains('tmark-lightbox-close')) {
        closeLightbox();
      }
    });
  ...
  function openLightbox(src, alt) {
    if (!lightbox) return;
    const img = lightbox.querySelector('.tmark-lightbox-img');
    img.src = src;
    img.alt = alt || '';
    lightbox.classList.add('visible');
    document.body.style.overflow = 'hidden';
    lightbox.querySelector('.tmark-lightbox-close').focus();
  }
    ...
    refs.forEach((ref, i) => {
      ...
      ref.addEventListener('click', (e) => {
        ...
        if (def) {
          ...
          def.classList.add('tmark-highlight');
          setTimeout(() => def.classList.remove('tmark-highlight'), 2000);
        }
      });
    });
    ...
    defs.forEach((def, i) => {
      ...
      backref.addEventListener('click', (e) => {
        ...
        if (ref) {
          ...
          ref.classList.add('tmark-highlight');
          setTimeout(() => ref.classList.remove('tmark-highlight'), 2000);
        }
      });
    });
  }
  ...
  function improveAccessibility() {
    ...
    const toggle = document.querySelector('.tmark-reader-toggle');
    if (toggle) {
      toggle.setAttribute('aria-label', 'Open reader settings');
      toggle.setAttribute('aria-expanded', 'false');
      toggle.setAttribute('aria-controls', 'tmark-reader-panel');
    }

    // Add id and role to panel
    if (panel) {
      panel.id = 'tmark-reader-panel';
