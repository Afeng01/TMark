...
describe("compositionGuard scheduleImeCleanup", () => {
  ...
  it("deletes exact preedit pinyin when compositionend data is empty but CJK was committed in place", () => {
    mockGetImeCleanupPrefixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditPrefixLength.mockReturnValue(8);

    const events = getFullPlugin();
    const deleteSpy = vi.fn().mockReturnThis();
    const setMetaSpy = vi.fn().mockReturnThis();
    const mockResolve = (_pos: number) => ({
      depth: 1,
      node: (d: number) => ({ type: { name: d === 1 ? "heading" : "doc" } }),
    ...
    events.compositionstart(mockView);
    events.compositionupdate(mockView, { data: "di'yi'bj" });
  ...
  it("deletes exact preedit pinyin before compositionStartPos when WebKit reports the start after preedit", () => {
    mockGetImeCleanupPrefixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditPrefixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditSuffixLength.mockReturnValue(8);
      ...
      state: {
        ...
        doc: {
          ...
          textBetween: vi.fn((from: number, to: number) => {
            if (from === 1 && to === 9) return "wo'zl'ze";
            if (from === 9) return "我在这";
            return "";
          }),
          content: { size: 40 },
        },
    ...
    events.compositionstart(mockView);
    events.compositionupdate(mockView, { data: "wo'zl'ze" });
    events.compositionend(mockView, { data: "我在这" });
  ...
  it("deletes only the pinyin range when compositionStartPos is already after committed CJK", () => {
    mockGetImeCleanupPrefixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditPrefixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditSuffixLength.mockReturnValue(null);
    mockGetImeCleanupPreeditRangeBeforeCursor.mockReturnValue({ fromOffset: 0, toOffset: 8 });
    ...
    const mockView = {
      state: {
        ...
        doc: {
          resolve: mockResolve,
          textBetween: vi.fn((from: number, to: number) => {
            if (from === 1 && to === 12) return "wo'zl'ze我在这";
            if (from === 12) return "";
            return "";
          }),
          content: { size: 40 },
    ...
    events.compositionstart(mockView);
    events.compositionupdate(mockView, { data: "wo'zl'ze我在这" });
    events.compositionend(mockView, { data: "我在这" });
