...
describe("workspaceStore", () => {
  ...
  describe("openWorkspace", () => {
    it("sets rootPath and isWorkspaceMode", () => {
      ...
      expect(state.workspaceRoots).toEqual(["/path/to/project"]);
  ...
  describe("closeWorkspace", () => {
    it("resets all workspace state", () => {
      const store = useWorkspaceStore.getState();
      store.openWorkspace("/path/to/project");
      store.addWorkspaceRoot("/another/project");
  ...
  describe("multiple workspace roots", () => {
    it("adds unrelated roots without replacing the active root", () => {
      const store = useWorkspaceStore.getState();
      store.openWorkspace("/path/to/project");
      store.addWorkspaceRoot("/other/notes");

      const state = useWorkspaceStore.getState();
      expect(state.rootPath).toBe("/path/to/project");
      expect(state.workspaceRoots).toEqual(["/path/to/project", "/other/notes"]);
      expect(state.isWorkspaceMode).toBe(true);
    });
    ...
    it("does not add duplicate roots", () => {
      const store = useWorkspaceStore.getState();
      store.openWorkspace("/path/to/project");
      store.addWorkspaceRoot("/path/to/project");

      expect(useWorkspaceStore.getState().workspaceRoots).toEqual(["/path/to/project"]);
    });

    it("promotes the next root when removing the active root", () => {
      const store = useWorkspaceStore.getState();
      store.openWorkspace("/path/to/project");
      store.addWorkspaceRoot("/other/notes");
      store.removeWorkspaceRoot("/path/to/project");

      const state = useWorkspaceStore.getState();
      expect(state.rootPath).toBe("/other/notes");
      expect(state.workspaceRoots).toEqual(["/other/notes"]);
      expect(state.isWorkspaceMode).toBe(true);
    });
  });
  ...
  describe("bootstrapConfig", () => {
    ...
    it("recovers legacy persisted state without workspaceRoots", () => {
      useWorkspaceStore.setState({
        rootPath: "/legacy/path",
        workspaceRoots: undefined as unknown as string[],
        isWorkspaceMode: true,
        config: null,
      });

      expect(() => useWorkspaceStore.getState().bootstrapConfig(null)).not.toThrow();

      const state = useWorkspaceStore.getState();
      expect(state.workspaceRoots).toEqual(["/legacy/path"]);
      expect(state.config?.excludeFolders).toEqual(DEFAULT_EXCLUDED_FOLDERS);
    });
  });
