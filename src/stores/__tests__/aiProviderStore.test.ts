...
describe("aiProviderStore", () => {
  ...
  it("activates Alice CLI provider", () => {
    useAiProviderStore.getState().activateProvider("alice");
    expect(useAiProviderStore.getState().activeProvider).toBe("alice");
  });
  ...
  describe("detectProviders", () => {
    it("populates cliProviders from Rust detection", async () => {
      vi.mocked(invoke).mockImplementation(async (cmd: string) => {
        if (cmd === "detect_ai_providers") {
          return [
            ...
            { type: "alice", name: "Alice", command: "alice-cli", available: true, path: "/Users/me/.local/bin/alice-cli" },
          ];
        }
        if (cmd === "read_env_api_keys") return {};
        return undefined;
      ...
      expect(cliProviders).toHaveLength(3);
      ...
      expect(cliProviders[2]).toEqual({
        type: "alice",
        name: "Alice",
        command: "alice-cli",
        available: true,
        path: "/Users/me/.local/bin/alice-cli",
      });
    });
