...
describe("getImeCleanupPreeditPrefixLength", () => {
  it("computes cleanup prefix from exact preedit when committed CJK follows it", () => {
    expect(getImeCleanupPreeditPrefixLength("di'yi'bj第一版/封禁", "di'yi'bj")).toBe(8);
  });

  it("uses the pinyin portion when the final compositionupdate contains preedit plus committed CJK", () => {
    expect(getImeCleanupPreeditPrefixLength("wo'zl'ze我在这", "wo'zl'ze我在这")).toBe(8);
  });

  it("preserves existing English when the text does not start with the exact preedit", () => {
    expect(getImeCleanupPreeditPrefixLength("Chapter di'yi'bj第一版/封禁", "di'yi'bj")).toBeNull();
  });

  it("skips cleanup when no CJK text follows the preedit", () => {
    expect(getImeCleanupPreeditPrefixLength("di'yi'bj draft", "di'yi'bj")).toBeNull();
  });

  it("skips cleanup when preedit is not pinyin-like", () => {
    expect(getImeCleanupPreeditPrefixLength("di-yi第一版", "di-yi")).toBeNull();
  });
});

// ---- getImeCleanupPreeditSuffixLength ----

describe("getImeCleanupPreeditSuffixLength", () => {
  it("computes cleanup length when exact preedit sits immediately before committed CJK", () => {
    expect(getImeCleanupPreeditSuffixLength("wo'zl'ze", "我在这", "wo'zl'ze")).toBe(8);
  });

  it("preserves existing English when it does not end with the exact preedit", () => {
    expect(getImeCleanupPreeditSuffixLength("Chapter wo", "我", "wo'zl'ze")).toBeNull();
  });

  it("skips cleanup when no CJK text follows the cursor", () => {
    expect(getImeCleanupPreeditSuffixLength("wo'zl'ze", " draft", "wo'zl'ze")).toBeNull();
  });

  it("skips cleanup when preedit is not pinyin-like", () => {
    expect(getImeCleanupPreeditSuffixLength("di-yi", "第一版", "di-yi")).toBeNull();
  });
});

// ---- getImeCleanupPreeditRangeBeforeCursor ----

describe("getImeCleanupPreeditRangeBeforeCursor", () => {
  it("returns the immediate preedit range when committed CJK is after the cursor", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("wo'zl'ze", "我在这", "wo'zl'ze", "我在这")).toEqual({
      fromOffset: 0,
      toOffset: 8,
    });
  });

  it("returns only the pinyin range when the cursor is already after committed CJK", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("wo'zl'ze我在这", "", "wo'zl'ze我在这", "我在这")).toEqual({
      fromOffset: 0,
      toOffset: 8,
    });
  });

  it("can infer apostrophe-separated pinyin from text when composition events expose only CJK", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("wo'zl'ze我在这", "", "", "我在这")).toEqual({
      fromOffset: 0,
      toOffset: 8,
    });
    expect(getImeCleanupPreeditRangeBeforeCursor("wo'zl'ze我在这", "", "我在这", "我在这")).toEqual({
      fromOffset: 0,
      toOffset: 8,
    });
  });

  it("does not infer ordinary English as pinyin when no preedit was captured", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("Title我在这", "", "", "我在这")).toBeNull();
  });

  it("preserves existing English before the exact preedit plus committed CJK", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("Title wo'zl'ze我在这", "", "wo'zl'ze我在这", "我在这")).toEqual({
      fromOffset: 6,
      toOffset: 14,
    });
  });

  it("skips cleanup when no committed CJK is adjacent to the preedit", () => {
    expect(getImeCleanupPreeditRangeBeforeCursor("wo'zl'ze draft", "", "wo'zl'ze", "")).toBeNull();
  });
});
