...
//!
//! Discovers which CLI AI providers (claude, codex, gemini, alice) are installed
...
mod tests {
    ...
    #[test]
    fn detects_alice_cli_provider_entry() {
        let providers = detect_ai_providers();
        let alice = providers
            .iter()
            .find(|provider| provider.provider_type == "alice")
            .expect("Alice CLI provider should be part of detection list");

        assert_eq!(alice.name, "Alice");
        assert_eq!(alice.command, "alice-cli");
    }
}
