...
async fn dispatch_to_provider(
    ...
    if max_tokens.is_some() && matches!(provider, "claude" | "codex" | "gemini" | "alice") {
        log::warn!(
...
mod tests {
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn alice_cli_provider_collects_text() {
        let cancel = CancellationToken::new();
        let result = run_ai_prompt_collect(
            cancel,
            "alice",
        ...
        assert!(result.is_ok(), "expected Ok got {:?}", result);
        let text = result.unwrap();
        assert!(text.contains("hello alice"), "expected echoed prompt in {}", text);
    }
}
