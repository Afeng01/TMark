/**
 * WebSocketBridge - Real WebSocket connection to TMark.
 *
 * Implements the Bridge interface for production use.
 * Connects to TMark's WebSocket server and relays MCP commands.
 */
...
export interface WebSocketBridgeConfig {
  ...
  maxQueueSize?: number;
  /** Client identity for TMark to identify who is connecting */
...
/**
 * WebSocketBridge connects to TMark via WebSocket.
 */
export class WebSocketBridge implements Bridge {
  ...
  /**
   * Connect to TMark WebSocket server.
   */
  async connect(): Promise<void> {
    ...
    if (port === undefined) {
      ...
      // Port file not present yet (TMark hasn't started). If autoReconnect is
      // enabled, schedule a retry so the sidecar keeps polling until TMark
      ...
      throw new Error(
        'Cannot determine TMark port. Is TMark running? ' +
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.on('open', () => {
          ...
          // Send auth token if available (required by TMark bridge)
  ...
  async send<T = unknown>(
    ...
    if (!this.isConnected()) {
      ...
      throw new Error('Not connected to TMark');
    }
