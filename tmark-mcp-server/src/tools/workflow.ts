 ...
 * YAML write loses comments and anchors; TMark's CST mutators preserve
...
export function registerWorkflowTool(server: TMarkMcpServer): void {
  server.registerTool(
    {
      name: 'workflow',
      description:
    ...
    async (args) => {
      ...
      if (action === 'apply_patch') {
        if (!Array.isArray(args.patches)) {
          return TMarkMcpServer.errorResult('patches (array) is required');
        }
        ...
        return TMarkMcpServer.successJsonResult(data);
      }
      if (action === 'validate') {
        const data = await server.sendBridgeRequest({
      ...
      return TMarkMcpServer.errorResult(
