...
export function registerDocumentTool(server: TMarkMcpServer): void {
  server.registerTool(
    {
      name: 'document',
      description:
    ...
    async (args) => {
      ...
      if (action === 'write') {
        if (typeof args.content !== 'string') {
          return TMarkMcpServer.errorResult('content (string) is required');
        }
        const data = await server.sendBridgeRequest({
      ...
      if (action === 'transform') {
        if (typeof args.kind !== 'string') {
          return TMarkMcpServer.errorResult('kind (string) is required');
        }
        const data = await server.sendBridgeRequest({
