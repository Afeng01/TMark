...
export function registerWorkspaceTool(server: TMarkMcpServer): void {
  server.registerTool(
    {
      name: 'workspace',
      description:
      ...
      switch (action) {
        ...
        case 'open': {
          if (typeof args.filePath !== 'string') {
            return TMarkMcpServer.errorResult('filePath (string) is required');
          }
          const data = await server.sendBridgeRequest({
        ...
        case 'close': {
          if (typeof args.tabId !== 'string') {
            return TMarkMcpServer.errorResult('tabId (string) is required');
          }
          const data = await server.sendBridgeRequest({
