#!/usr/bin/env node
/**
 * TMark MCP Server — pruned 5-tool surface.
 *
 * Exposes TMark to AI assistants via the MCP protocol with five
 ...
 *
 * Usage:
 *   npx @tmark/mcp-server
 *   node dist/index.js
 *
 * The server communicates with TMark via WebSocket bridge on
...
export type { TMarkMcpServerConfig, ToolArgs } from './server.js';
...
import { TMarkMcpServer } from './server.js';
...
/**
 * Create a fully configured TMark MCP server with the pruned 5-tool
 * surface registered (selection re-added per ADR-7).
 */
export function createTMarkMcpServer(bridge: Bridge): TMarkMcpServer {
  const server = new TMarkMcpServer({ bridge });
 ...
 * through `tmark.session.get_state`, which gives the AI everything the
 * deleted tmark://document/* and tmark://windows/* resources used to
