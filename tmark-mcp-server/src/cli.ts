#!/usr/bin/env node
/**
 * TMark MCP Server CLI - Sidecar entry point.
 *
 * This is the entry point for the bundled sidecar binary.
 * It starts the MCP server and connects to TMark via WebSocket.
 *
 * Port Discovery:
 * - TMark writes its bridge port to the app data directory (mcp-port file)
 * - This sidecar reads the port from that file automatically
 * - No user configuration needed!
 *
 * Usage:
 *   tmark-mcp-server              # Auto-discovers port from app data directory
 *   tmark-mcp-server --port 9223  # Manual port override (legacy)
 *   tmark-mcp-server --version    # Print version and exit
 *   tmark-mcp-server --health-check # Run self-test and exit
 */
 ...
 * Validates that the binary is functional without requiring TMark connection.
 */
...
async function runHealthCheck(): Promise<void> {
  try {
    ...
    const { createTMarkMcpServer } = await import('./index.js');

    // 2. Create a mock bridge that doesn't connect (implements Bridge interface)
    const mockBridge = {
      send: async (): Promise<never> => {
        throw new Error('Health check mode - no TMark connection');
      },
    ...
    const server = createTMarkMcpServer(mockBridge);
...
import { createTMarkMcpServer, EXPECTED_TOOL_COUNT } from './index.js';
 ...
 */
const APP_IDENTIFIER = process.env.TMARK_APP_IDENTIFIER || 'app.tmark';
...
/**
 * Read port and auth token from the port file written by TMark.
...
function readPortFromFile(): number | undefined {
  ...
  try {
    ...
    if (!isNotFoundError(err)) {
      ...
      if (process.env.TMARK_DEBUG) {
        console.error('[TMark MCP] Failed to read port file:', err);
      }
    }
    // ENOENT is expected if TMark hasn't started yet
  }
...
const logger = {
  ...
  warn: (message: string, ...args: unknown[]) => {
    console.error('[TMark MCP] WARN:', message, ...args);
  },
  error: (message: string, ...args: unknown[]) => {
    console.error('[TMark MCP] ERROR:', message, ...args);
  },
};
...
async function main(): Promise<void> {
  ...
  // Create WebSocket bridge to connect to TMark
  ...
  // Create the TMark MCP server with all tools
  const tmarkServer = createTMarkMcpServer(bridge);
  const allTools = tmarkServer.listTools();

  // Create high-level MCP server
  const mcpServer = new McpServer(
    {
      name: 'tmark-mcp-server',
      version: '0.1.0',
    },
    {
      capabilities: {
  ...
  for (const tool of allTools) {
    ...
    mcpServer.registerTool(
      ...
      createToolHandler(tool.name, (name, args) => tmarkServer.callTool(name, args))
    );
  }

  // Register all resources from the TMark server
  for (const resource of tmarkServer.listResources()) {
    mcpServer.registerResource(
      ...
      createResourceHandler(resource.uri, (uri) => tmarkServer.readResource(uri))
    );
  }

  // Connect to TMark first (errors logged by bridge)
  try {
...
process.on('unhandledRejection', (reason) => {
  console.error('[TMark MCP] Unhandled rejection:', reason);
  // Don't exit — let reconnection recover if possible
});

// Catch uncaught synchronous exceptions
process.on('uncaughtException', (error) => {
  console.error('[TMark MCP] Uncaught exception:', error);
  process.exit(1);
});
...
if (!process.argv.includes('--health-check')) {
  main().catch((error) => {
    console.error('[TMark MCP Server] Fatal error:', error);
    process.exit(1);
  });
}
