...
describe('toMcpContents', () => {
  it('should convert resource items', () => {
    const items = [{ uri: 'tmark://doc', text: 'content', mimeType: 'text/markdown' }];
    expect(toMcpContents(items)).toEqual([
      { uri: 'tmark://doc', text: 'content', mimeType: 'text/markdown' },
    ]);
  });

  it('should filter out items without text', () => {
    const items = [
      { uri: 'tmark://a', text: 'keep' },
      { uri: 'tmark://b' },
    ];
    const result = toMcpContents(items as Array<{ uri: string; text?: string }>);
    expect(result).toHaveLength(1);
    expect(result[0].uri).toBe('tmark://a');
  });
...
describe('createResourceHandler', () => {
  it('should return resource contents on success', async () => {
    const readResource = vi.fn().mockResolvedValue({
      contents: [{ uri: 'tmark://doc', text: 'content' }],
    });

    const handler = createResourceHandler('tmark://doc', readResource);
    const result = await handler();

    expect(readResource).toHaveBeenCalledWith('tmark://doc');
    expect(result.contents).toEqual([{ uri: 'tmark://doc', text: 'content' }]);
  });
