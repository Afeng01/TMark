/**
 * Tests for TMarkMcpServer class.
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';
import { TMarkMcpServer, resolveWindowId } from '../../src/server.js';
import { MockBridge } from '../mocks/mockBridge.js';

describe('TMarkMcpServer', () => {
  let bridge: MockBridge;
  let server: TMarkMcpServer;

  beforeEach(() => {
    bridge = new MockBridge();
    server = new TMarkMcpServer({ bridge });
  });

  describe('constructor', () => {
    it('should use default name and version', () => {
      const info = server.getServerInfo();
      expect(info.name).toBe('tmark');
      expect(info.version).toBe('0.1.0');
    });

    it('should use custom name and version', () => {
      const customServer = new TMarkMcpServer({
        bridge,
        name: 'custom-server',
        version: '2.0.0',
      });
  ...
  describe('registerResource', () => {
    it('should register a resource', () => {
      ...
      expect(server.resources.has('tmark://test')).toBe(true);
  ...
  describe('listResources', () => {
    ...
    it('should return all resource definitions', () => {
      server.registerResource(
        { uri: 'tmark://a', name: 'A', description: 'Resource A' },
        async () => ({ contents: [] })
      );
      server.registerResource(
        { uri: 'tmark://b', name: 'B', description: 'Resource B' },
        async () => ({ contents: [] })
      );
  ...
  describe('readResource', () => {
    it('should call resource handler', async () => {
      const handler = vi.fn().mockResolvedValue({
        contents: [{ uri: 'tmark://test', text: 'content' }],
      });

      server.registerResource(
        { uri: 'tmark://test', name: 'Test', description: 'Test' },
        handler
      );

      await server.readResource('tmark://test');

      expect(handler).toHaveBeenCalledWith('tmark://test');
    });
    ...
    it('should return result from handler', async () => {
      server.registerResource(
        { uri: 'tmark://test', name: 'Test', description: 'Test' },
        async () => ({
          contents: [{ uri: 'tmark://test', text: 'hello world' }],
        })
      );

      const result = await server.readResource('tmark://test');

      expect(result.contents[0].text).toBe('hello world');
    });

    it('should throw for unknown resource', async () => {
      await expect(server.readResource('tmark://unknown')).rejects.toThrow(
        'Unknown resource'
      );
    });
    ...
    it('should wrap handler errors with resource URI context', async () => {
      server.registerResource(
        { uri: 'tmark://failing', name: 'Failing', description: 'Fails' },
        async () => {
          throw new Error('Handler exploded');
        }
      );

      await expect(server.readResource('tmark://failing')).rejects.toThrow(
        'Resource error (tmark://failing): Handler exploded'
      );
    });

    it('should handle non-Error thrown values in handler', async () => {
      server.registerResource(
        { uri: 'tmark://weird', name: 'Weird', description: 'Throws string' },
        async () => {
          throw 'String error';
        }
      );

      await expect(server.readResource('tmark://weird')).rejects.toThrow(
        'Resource error (tmark://weird): String error'
      );
    });
  });
  ...
  describe('static helper methods', () => {
    describe('successResult', () => {
      it('should create text result', () => {
        const result = TMarkMcpServer.successResult('hello');
    ...
    describe('successJsonResult', () => {
      it('should create JSON text result', () => {
        const result = TMarkMcpServer.successJsonResult({ key: 'value' });
      ...
      it('should format JSON with indentation', () => {
        const result = TMarkMcpServer.successJsonResult({ a: 1 });
    ...
    describe('errorResult', () => {
      it('should create error result', () => {
        const result = TMarkMcpServer.errorResult('Something went wrong');
    ...
    describe('resourceResult', () => {
      it('should create resource result', () => {
        const result = TMarkMcpServer.resourceResult(
          'tmark://doc',
          'content',
          'text/markdown'
        );
        ...
        expect(result.contents[0].uri).toBe('tmark://doc');
        expect(result.contents[0].text).toBe('content');
        expect(result.contents[0].mimeType).toBe('text/markdown');
      });

      it('should work without mimeType', () => {
        const result = TMarkMcpServer.resourceResult('tmark://doc', 'content');
