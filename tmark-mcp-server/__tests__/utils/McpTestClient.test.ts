...
describe('McpTestClient', () => {
  ...
  describe('resource operations', () => {
    it('should list available resources', () => {
      registerMockResource(
        ...
        async () => ({ contents: [{ uri: 'tmark://test', text: 'content' }] })
      );

      const resources = client.listResources();

      expect(resources).toHaveLength(1);
      expect(resources[0].uri).toBe('tmark://test');
    });
    ...
    it('should get resource definition by URI', () => {
      ...
      const resource = client.getResource('tmark://document');
    ...
    it('should return undefined for unknown resource', () => {
      const resource = client.getResource('tmark://nonexistent');
    ...
    it('should read resource and return result', async () => {
      ...
      const result = await client.readResource('tmark://content');
    ...
    it('should record resource read history', async () => {
      registerMockResource(
        ...
        async () => ({ contents: [{ uri: 'tmark://doc', text: 'text' }] })
      );

      await client.readResource('tmark://doc');
      await client.readResource('tmark://doc');
    ...
    it('should throw for unknown resource', async () => {
      await expect(client.readResource('tmark://unknown')).rejects.toThrow(
        'Unknown resource'
      );
    });
  });
  ...
  describe('assertions', () => {
    beforeEach(() => {
      ...
      registerMockResource(
        ...
        async () => ({ contents: [{ uri: 'tmark://data', text: '{}' }] })
      );
    });
    ...
    it('should pass assertResourceRead when resource was read', async () => {
      await client.readResource('tmark://data');

      expect(() => client.assertResourceRead('tmark://data')).not.toThrow();
    });

    it('should fail assertResourceRead when resource was not read', () => {
      expect(() => client.assertResourceRead('tmark://data')).toThrow(
        'Expected resource "tmark://data" to be read'
      );
    });
  });
  ...
  describe('content helpers', () => {
    ...
    it('should extract text from resource result', () => {
      const result: ResourceReadResult = {
        contents: [
          { uri: 'tmark://doc', text: 'Document content' },
        ],
      };
    ...
    it('should extract text from specific URI in resource result', () => {
      const result: ResourceReadResult = {
        contents: [
          { uri: 'tmark://first', text: 'First' },
          { uri: 'tmark://second', text: 'Second' },
        ],
      };

      expect(McpTestClient.getResourceText(result, 'tmark://second')).toBe('Second');
    });

    it('should parse JSON from resource result', () => {
      const result: ResourceReadResult = {
        contents: [
          { uri: 'tmark://data', text: '{"items": [1, 2, 3]}' },
        ],
      };
  ...
  describe('history management', () => {
    it('should clear history', async () => {
      ...
      await client.readResource('tmark://res');
