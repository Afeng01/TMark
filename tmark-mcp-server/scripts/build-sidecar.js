#!/usr/bin/env node
/**
 * Build script for TMark MCP Server sidecar binary.
 *
 ...
 *
 * Output format: tmark-mcp-server-{target-triple}
 *   - tmark-mcp-server-aarch64-apple-darwin (M1/M2 Mac)
 *   - tmark-mcp-server-x86_64-apple-darwin (Intel Mac)
 *   - tmark-mcp-server-x86_64-pc-windows-msvc.exe (Windows)
 *   - tmark-mcp-server-x86_64-unknown-linux-gnu (Linux)
 */
...
async function buildForTarget(targetKey) {
  ...
  const outputName = `tmark-mcp-server-${target.triple}${ext}`;
...
async function main() {
  ...
  console.log('TMark MCP Server Sidecar Builder');
  console.log('================================\n');
