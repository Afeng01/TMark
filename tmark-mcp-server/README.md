...
MCP (Model Context Protocol) server for TMark - expose Tiptap editor APIs to AI assistants.

## Installation

```bash
npm install @tmark/mcp-server
# or
pnpm add @tmark/mcp-server
```

## Quick Start

```typescript
import { WebSocketBridge, TMarkMcpServer, createTMarkMcpServer } from '@tmark/mcp-server';

// Create bridge connection to TMark
...
const server = createTMarkMcpServer(bridge);

// Or create server manually for custom tool registration
const customServer = new TMarkMcpServer({ bridge });
```
...
const unsubscribe = bridge.onConnectionChange((connected) => {
  if (connected) {
    console.log('Connected to TMark');
  } else {
    console.log('Disconnected from TMark');
  }
});
...
The bridge includes built-in rate limiting to prevent overwhelming TMark:
...
- Node.js >= 18
- TMark running with MCP bridge enabled
